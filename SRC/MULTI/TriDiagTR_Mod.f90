MODULE TriTR_1D_Mod
  ...
  TYPE TriTRDiag1D_T
    ...
    TYPE(TriTRDiag1D_T), POINTER :: Coarse=>NULL()
    TYPE(TriTRDiag1D_T), POINTER :: Fine=>NULL()
  END TYPE TriTRDiag1D_T
  ...
  END INTERFACE Solve
  INTERFACE MatVecTR
    MODULE PROCEDURE MatVecTR_1D
  END INTERFACE MatVecTR
  INTERFACE Compute
    MODULE PROCEDURE Compute_1D
  END INTERFACE Compute
CONTAINS

SUBROUTINE Compute_1D(A,F,FG,Vol,DTU,DUU,DUT,DUR)

  TYPE (TriTRDiag1D_T) :: A
  REAL(RealKind) :: F(:),FG(:),Vol(:)
  REAL(RealKind) :: DTU(:),DUU(:),DUT(:),DUR(:)
...
!     l_2 a_3 u_3
  ...
  A%d(1,2,1)=A%d(1,2,1)+(beta0*dtp)**2*Fac1_L*F(1)*DTU(1)*DUU(1)*FG(1)*DUR(1)/(Vol(1)+Eps)
  A%d(2,1,1)=A%d(2,1,1)+(beta0*dtp)**2*Fac1_L*F(1)*DUU(1)*FG(1)*DUT(1)/(Vol(1)+Eps)
  A%d(2,2,1)=A%d(2,2,1)+(beta0*dtp)**2*Fac1_L*F(1)*DUU(1)*FG(1)*DUR(1)/(Vol(1)+Eps)
  ...
  DO i=1,n1-1
    ...
    A%u(1,2,i)=-(beta0*dtp)**2*2.0d0*F(i+1)*DTU(i+1)*DUU(i+1)*FG(i+1)*DUR(i+1)/(Vol(i)+Vol(i+1)+Eps)
    A%u(2,1,i)=-(beta0*dtp)**2*2.0d0*F(i+1)*DUU(i+1)*FG(i+1)*DUT(i+1)/(Vol(i)+Vol(i+1)+Eps)
    A%u(2,2,i)=-(beta0*dtp)**2*2.0d0*F(i+1)*DUU(i+1)*FG(i+1)*DUR(i+1)/(Vol(i)+Vol(i+1)+Eps)
    A%l(1,1,i)=-(beta0*dtp)**2*2.0d0*F(i+1)*DTU(i+1)*DUU(i+1)*FG(i+1)*DUT(i)/(Vol(i)+Vol(i+1)+Eps)
    A%l(1,2,i)=-(beta0*dtp)**2*2.0d0*F(i+1)*DTU(i+1)*DUU(i+1)*FG(i+1)*DUR(i)/(Vol(i)+Vol(i+1)+Eps)
    A%l(2,1,i)=-(beta0*dtp)**2*2.0d0*F(i+1)*DUU(i+1)*FG(i+1)*DUT(i)/(Vol(i)+Vol(i+1)+Eps)
    A%l(2,2,i)=-(beta0*dtp)**2*2.0d0*F(i+1)*DUU(i+1)*FG(i+1)*DUR(i)/(Vol(i)+Vol(i+1)+Eps)
...
!   A%d(i)=A%d(i)-A%l(i)
    ...
    A%d(1,2,i)=A%d(1,2,i)+(beta0*dtp)**2*2.0d0*F(i+1)*DTU(i+1)*DUU(i+1)*FG(i+1)*DUR(i)/(Vol(i)+Vol(i+1)+Eps)
    A%d(2,1,i)=A%d(2,1,i)+(beta0*dtp)**2*2.0d0*F(i+1)*DUU(i+1)*FG(i+1)*DUT(i)/(Vol(i)+Vol(i+1)+Eps)
    A%d(2,2,i)=A%d(2,2,i)+(beta0*dtp)**2*2.0d0*F(i+1)*DUU(i+1)*FG(i+1)*DUR(i)/(Vol(i)+Vol(i+1)+Eps)
!   A%d(i+1)=A%d(i+1)-A%u(i)
    A%d(1,1,i+1)=A%d(1,1,i+1)+(beta0*dtp)**2*2.0d0*F(i+1)*DTU(i+1)*DUU(i+1)*FG(i+1)*DUT(i+1)/(Vol(i)+Vol(i+1)+Eps)
    A%d(1,2,i+1)=A%d(1,2,i+1)+(beta0*dtp)**2*2.0d0*F(i+1)*DTU(i+1)*DUU(i+1)*FG(i+1)*DUR(i+1)/(Vol(i)+Vol(i+1)+Eps)
    A%d(2,1,i+1)=A%d(2,1,i+1)+(beta0*dtp)**2*2.0d0*F(i+1)*DUU(i+1)*FG(i+1)*DUT(i+1)/(Vol(i)+Vol(i+1)+Eps)
    A%d(2,2,i+1)=A%d(2,2,i+1)+(beta0*dtp)**2*2.0d0*F(i+1)*DUU(i+1)*FG(i+1)*DUR(i+1)/(Vol(i)+Vol(i+1)+Eps)
    IF (.NOT.GradFull) THEN
  ...
  A%d(1,2,n1)=A%d(1,2,n1)+(beta0*dtp)**2*Fac1_R*F(n1+1)*DTU(n1+1)*DUU(n1+1)*FG(n1+1)*DUR(n1)/(Vol(n1)+Eps)
  A%d(2,1,n1)=A%d(2,1,n1)+(beta0*dtp)**2*Fac1_R*F(n1+1)*DUU(n1+1)*FG(n1+1)*DUT(n1)/(Vol(n1)+Eps)
  A%d(2,2,n1)=A%d(2,2,n1)+(beta0*dtp)**2*Fac1_R*F(n1+1)*DUU(n1+1)*FG(n1+1)*DUR(n1)/(Vol(n1)+Eps)
...
MODULE TriTR_2D_Mod
  ...
  TYPE TriTRDiag2D_T
    INTEGER :: n1=0
    INTEGER :: n2=0
    TYPE(TriTRDiag1D_T), POINTER :: d(:)=>NULL()
    TYPE(TriTRDiag2D_T), POINTER :: Coarse=>NULL()
    TYPE(TriTRDiag2D_T), POINTER :: Fine=>NULL()
    REAL(RealKind), POINTER :: u(:,:,:,:)=>NULL()
    REAL(RealKind), POINTER :: l(:,:,:,:)=>NULL()
    REAL(RealKind), POINTER :: Res(:,:,:)=>NULL()
    REAL(RealKind), POINTER :: ResCoarse(:,:,:)=>NULL()
    REAL(RealKind), POINTER :: xCoarse(:,:,:)=>NULL()
  END TYPE TriTRDiag2D_T
  TYPE TriTRDiag2DP_T
    TYPE(TriTRDiag2D_T), POINTER :: P=>NULL()
  END TYPE TriTRDiag2DP_T
  ...
  INTERFACE MatVecTR
    MODULE PROCEDURE MatVecTR_2D
  END INTERFACE MatVecTR
  INTERFACE RestrictTR
    MODULE PROCEDURE RestrictTR_2D
  END INTERFACE RestrictTR
  INTERFACE ProlongateTR
    MODULE PROCEDURE ProlongateTR_2D
  END INTERFACE ProlongateTR
  INTERFACE Compute
...
SUBROUTINE Allocate_2D(A,n2,n1)
  ...
  IF (PRESENT(n2)) THEN
    ...
    IF (PRESENT(n1)) THEN
      ...
      END DO
      IF (.NOT.ASSOCIATED(A%u)) THEN
        ALLOCATE(A%u(1:2,1:2,n1,n2-1))
      END IF
      A%u=Zero
      IF (.NOT.ASSOCIATED(A%l)) THEN
        ALLOCATE(A%l(1:2,1:2,n1,n2-1))
      END IF
      A%l=Zero
...
SUBROUTINE Compute_2D(A,F1,F2,F1G,F2G,Vol,DTU1,DTU2,DUU1,DUU2,DUT,DUR)

  TYPE (TriTRDiag2D_T) :: A
  REAL(RealKind) :: F1(:,:)
  REAL(RealKind) :: F2(:,:)
  REAL(RealKind) :: F1G(:,:)
  REAL(RealKind) :: F2G(:,:)
  ...
  IF (ASSOCIATED(A%u).AND.ASSOCIATED(A%l)) THEN
    CALL Compute(A%d(1),F1(:,1),F1G(:,1),Vol(:,1),DTU1(:,1),DUU1(:,1),DUT(:,1),DUR(:,1))
    DO i=1,n1
      A%d(1)%d(1,1,i)=A%d(1)%d(1,1,i)+(beta0*dtp)**2*Fac2_L*F2(i,1)*DTU2(i,1)*DUU2(i,1)*F2G(i,1)*DUT(i,1)/(Vol(i,1)+Eps)
      A%d(1)%d(1,2,i)=A%d(1)%d(1,2,i)+(beta0*dtp)**2*Fac2_L*F2(i,1)*DTU2(i,1)*DUU2(i,1)*F2G(i,1)*DUR(i,1)/(Vol(i,1)+Eps)
      A%d(1)%d(2,1,i)=A%d(1)%d(2,1,i)+(beta0*dtp)**2*Fac2_L*F2(i,1)*DUU2(i,1)*F2G(i,1)*DUT(i,1)/(Vol(i,1)+Eps)
      A%d(1)%d(2,2,i)=A%d(1)%d(2,2,i)+(beta0*dtp)**2*Fac2_L*F2(i,1)*DUU2(i,1)*F2G(i,1)*DUR(i,1)/(Vol(i,1)+Eps)
    END DO
    DO j=1,n2-1
      DO i=1,n1
        A%u(1,1,i,j)=-(beta0*dtp)**2*2.0d0*F2(i,j+1)*DTU2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUT(i,j+1)/(Vol(i,j)+Vol(i,j+1)+Eps)
        A%u(1,2,i,j)=-(beta0*dtp)**2*2.0d0*F2(i,j+1)*DTU2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUR(i,j+1)/(Vol(i,j)+Vol(i,j+1)+Eps)
        A%u(2,1,i,j)=-(beta0*dtp)**2*2.0d0*F2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUT(i,j+1)/(Vol(i,j)+Vol(i,j+1)+Eps)
        A%u(2,2,i,j)=-(beta0*dtp)**2*2.0d0*F2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUR(i,j+1)/(Vol(i,j)+Vol(i,j+1)+Eps)
        A%l(1,1,i,j)=-(beta0*dtp)**2*2.0d0*F2(i,j+1)*DTU2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUT(i,j)/(Vol(i,j)+Vol(i,j+1)+Eps)
        A%l(1,2,i,j)=-(beta0*dtp)**2*2.0d0*F2(i,j+1)*DTU2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUR(i,j)/(Vol(i,j)+Vol(i,j+1)+Eps)
        A%l(2,1,i,j)=-(beta0*dtp)**2*2.0d0*F2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUT(i,j)/(Vol(i,j)+Vol(i,j+1)+Eps)
        A%l(2,2,i,j)=-(beta0*dtp)**2*2.0d0*F2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUR(i,j)/(Vol(i,j)+Vol(i,j+1)+Eps)
!       A%d(j)%d(i)=A%d(j)%d(i)-A%e(i,j)
        ...
        A%d(j)%d(1,2,i)=A%d(j)%d(1,2,i) &
            +(beta0*dtp)**2*2.0d0*F2(i,j+1)*DTU2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUR(i,j)/(Vol(i,j)+Vol(i,j+1)+Eps)
        A%d(j)%d(2,1,i)=A%d(j)%d(2,1,i) &
            +(beta0*dtp)**2*2.0d0*F2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUT(i,j)/(Vol(i,j)+Vol(i,j+1)+Eps)
        A%d(j)%d(2,2,i)=A%d(j)%d(2,2,i) &
            +(beta0*dtp)**2*2.0d0*F2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUR(i,j)/(Vol(i,j)+Vol(i,j+1)+Eps)
      END DO
      CALL Compute(A%d(j+1),F1(:,j+1),F1G(:,j+1),Vol(:,j+1),DTU1(:,j+1),DUU1(:,j+1),DUT(:,j+1),DUR(:,j+1))
      DO i=1,n1
!       A%d(j+1)%d(i)=A%d(j+1)%d(i)-A%e(i,j)
        ...
        A%d(j+1)%d(1,2,i)=A%d(j+1)%d(1,2,i) &
            +(beta0*dtp)**2*2.0d0*F2(i,j+1)*DTU2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUR(i,j+1)/(Vol(i,j)+Vol(i,j+1)+Eps)
        A%d(j+1)%d(2,1,i)=A%d(j+1)%d(2,1,i) &
            +(beta0*dtp)**2*2.0d0*F2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUT(i,j+1)/(Vol(i,j)+Vol(i,j+1)+Eps)
        A%d(j+1)%d(2,2,i)=A%d(j+1)%d(2,2,i) &
            +(beta0*dtp)**2*2.0d0*F2(i,j+1)*DUU2(i,j+1)*F2G(i,j+1)*DUR(i,j+1)/(Vol(i,j)+Vol(i,j+1)+Eps)
      END DO
    END DO
    DO i=1,n1
      A%d(n2)%d(1,1,i)=A%d(n2)%d(1,1,i) &
                         +(beta0*dtp)**2*Fac2_R*F2(i,n2+1)*DTU2(i,n2+1)*DUU2(i,n2+1)*F2G(i,n2+1)*DUT(i,n2)/(Vol(i,n2)+Eps)
      A%d(n2)%d(1,2,i)=A%d(n2)%d(1,2,i) &
                         +(beta0*dtp)**2*Fac2_R*F2(i,n2+1)*DTU2(i,n2+1)*DUU2(i,n2+1)*F2G(i,n2+1)*DUR(i,n2)/(Vol(i,n2)+Eps)
      A%d(n2)%d(2,1,i)=A%d(n2)%d(2,1,i) &
                         +(beta0*dtp)**2*Fac2_R*F2(i,n2+1)*DUU2(i,n2+1)*F2G(i,n2+1)*DUT(i,n2)/(Vol(i,n2)+Eps)
      A%d(n2)%d(2,2,i)=A%d(n2)%d(2,2,i) &
                         +(beta0*dtp)**2*Fac2_R*F2(i,n2+1)*DUU2(i,n2+1)*F2G(i,n2+1)*DUR(i,n2)/(Vol(i,n2)+Eps)
    END DO
  ELSE
    CALL Compute(A%d(1),F1(:,1),F1G(:,1),Vol(:,1),DTU1(:,1),DUU1(:,1),DUT(:,1),DUR(:,1))
    DO i=1,n1
...
SUBROUTINE MatVecTR_2D(y,A,x)
  ...
  n1=A%n1
  n2=A%n2
  CALL MatVecTR(y(:,:,1),A%d(1),x(:,:,1))
  IF (n2>1) THEN
    DO i=1,n1
      y(1:2,i,1)=y(1:2,i,1)+Mult_2(A%u(1:2,1:2,i,1),x(1:2,i,2))    
    END DO
    DO j=2,n2-1
      CALL MatVecTR(y(:,:,j),A%d(j),x(:,:,j))
      DO i=1,n1
        y(1:2,i,j)=y(1:2,i,j)+Mult_2(A%l(1:2,1:2,i,j-1),x(1:2,i,j-1))+Mult_2(A%u(1:2,1:2,i,j),x(1:2,i,j+1))    
      END DO
    END DO
    CALL MatVecTR(y(:,:,n2),A%d(n2),x(:,:,n2))
    DO i=1,n1
      y(1:2,i,n2)=y(1:2,i,n2)+Mult_2(A%l(1:2,1:2,i,n2-1),x(1:2,i,n2-1))    
    END DO
  END IF

END SUBROUTINE MatVecTR_2D
...
SUBROUTINE GaussSeidel_2D(A,b,x,GaussIter)
  ...
  IF (A%n2>1) THEN
    DO iter=1,GaussIter
      ...
      DO j=1,A%n2-1
        DO i=1,A%n1
          x(1:2,i,j)=t(1:2,i)-Mult_2(A%u(1:2,1:2,i,j),x(1:2,i,j+1))
        END DO 
        CALL Solve(A%d(j),x(:,:,j))
        DO i=1,A%n1
          t(1:2,i)=b(1:2,i,j+1)-Mult_2(A%l(1:2,1:2,i,j),x(1:2,i,j))
        END DO 
      END DO
      x(:,:,A%n2)=t(:,:)
      CALL Solve(A%d(A%n2),x(:,:,A%n2))
      DO i=1,A%n1
        t(1:2,i)=b(1:2,i,A%n2-1)-Mult_2(A%u(1:2,1:2,i,A%n2-1),x(1:2,i,A%n2))
      END DO  
      DO j=A%n2-1,2,-1
        DO i=1,A%n1
          x(1:2,i,j)=t(1:2,i)-Mult_2(A%l(1:2,1:2,i,j-1),x(1:2,i,j-1))
        END DO 
        CALL Solve(A%d(j),x(:,:,j))
        DO i=1,A%n1
          t(1:2,i)=b(1:2,i,j-1)-Mult_2(A%u(1:2,1:2,i,j-1),x(1:2,i,j))
        END DO 
      END DO
    END DO
    x(:,:,1)=t(:,:)
...
SUBROUTINE Multigrid_2D(A,b,x)
  ...
  TYPE(TriTRDiag2D_T), TARGET :: A
  REAL(RealKind), TARGET :: b(:,:,:)
  REAL(RealKind), TARGET :: x(:,:,:)

  INTEGER :: i,j,n1,n2
  TYPE(TriTRDiag2D_T), POINTER :: ALoc
...
!     RestrictTR
      CALL MatVecTR(ALoc%Res,ALoc,xLoc)
      ALoc%Res=bLoc-ALoc%Res
      CALL RestrictTR(ALoc%Res,ALoc%ResCoarse)
!     Down
      bLoc=>ALoc%ResCoarse
      xLoc=>ALoc%xCoarse
      xLoc=Zero
...
!     Up
      ...
      n1=ALoc%n1
      n2=ALoc%n2
      CALL ProlongateTR(xLoc,ALoc%Res)
      IF (ASSOCIATED(ALoc%Fine)) THEN
...
SUBROUTINE RestrictTR_2D(Res,ResCoarse)
  ...
  DO jC=1,n2/2
    ...
    jF=jF+2
  END DO
END SUBROUTINE RestrictTR_2D

SUBROUTINE ProlongateTR_2D(xCoarse,x)
  ...
  DO jC=1,n2/2
    ...
    jF=jF+2
  END DO
! CALL ProlongateTR(XCoarse(:,n2C),x(:,n2))
  iF=1
  DO iC=1,n1/2
...
END SUBROUTINE ProlongateTR_2D
END MODULE TriTR_2D_Mod
...
MODULE TriTR_3D_Mod
  ...
  TYPE TriTRDiag3D_T
    ...
    INTEGER :: n2=0
    INTEGER :: n3=0
    TYPE(TriTRDiag2DP_T), POINTER :: d(:)=>NULL()
    REAL(RealKind), POINTER :: u(:,:,:,:,:)=>NULL()
    REAL(RealKind), POINTER :: l(:,:,:,:,:)=>NULL()
    TYPE(TriTRDiag3D_T), POINTER :: Coarse=>NULL()
    TYPE(TriTRDiag3D_T), POINTER :: Fine=>NULL()
    REAL(RealKind), POINTER :: Res(:,:,:,:)=>NULL()
    REAL(RealKind), POINTER :: ResCoarse(:,:,:,:)=>NULL()
    REAL(RealKind), POINTER :: xCoarse(:,:,:,:)=>NULL()
  END TYPE TriTRDiag3D_T
  ...
  END INTERFACE GaussSeidel
  INTERFACE MatVecTR
    MODULE PROCEDURE MatVecTR_3D
  END INTERFACE MatVecTR
  INTERFACE RestrictTR
    MODULE PROCEDURE RestrictTR_3D
  END INTERFACE RestrictTR
  INTERFACE ProlongateTR
    MODULE PROCEDURE ProlongateTR_3D
  END INTERFACE ProlongateTR
  INTERFACE Compute
  ...
  IF (PRESENT(n3)) THEN
    ...
    IF (PRESENT(n2)) THEN
      ...
      IF (PRESENT(n1)) THEN
        A%n1=n1
        IF (.NOT.ASSOCIATED(A%u)) THEN
          ALLOCATE(A%u(1:2,1:2,n1,n2,n3-1))
        END IF
        A%u=Zero
        IF (.NOT.ASSOCIATED(A%l)) THEN
          ALLOCATE(A%l(1:2,1:2,n1,n2,n3-1))
        END IF
        A%l=Zero
        IF (.NOT.ASSOCIATED(A%Res)) THEN
...
SUBROUTINE Compute_3D(A,F1,F2,F3,F1G,F2G,F3G,Vol,DTU1,DTU2,DTU3,DUU1,DUU2,DUU3,DUT,DUR)
  ...
  IF (ASSOCIATED(A%u).AND.ASSOCIATED(A%l)) THEN
    CALL Compute(A%d(1)%P,F1(:,:,1),F2(:,:,1),F1G(:,:,1),F2G(:,:,1),Vol(:,:,1), &
                 DTU1(:,:,1),DTU2(:,:,1),DUU1(:,:,1),DUU2(:,:,1),DUT(:,:,1),DUR(:,:,1))
    DO j=1,n2
      DO i=1,n1
        ...
        A%d(1)%P%d(j)%d(1,2,i)=A%d(1)%P%d(j)%d(1,2,i) &
           +(beta0*dtp)**2*Fac3_L*F3(i,j,1)*DTU3(i,j,1)*DUU3(i,j,1)*F3G(i,j,1)*DUR(i,j,1)/(Vol(i,j,1)+Eps)
        A%d(1)%P%d(j)%d(2,1,i)=A%d(1)%P%d(j)%d(2,1,i) &
           +(beta0*dtp)**2*Fac3_L*F3(i,j,1)*DUU3(i,j,1)*F3G(i,j,1)*DUT(i,j,1)/(Vol(i,j,1)+Eps)
        A%d(1)%P%d(j)%d(2,2,i)=A%d(1)%P%d(j)%d(2,2,i) &
           +(beta0*dtp)**2*Fac3_L*F3(i,j,1)*DUU3(i,j,1)*F3G(i,j,1)*DUR(i,j,1)/(Vol(i,j,1)+Eps)
      END DO
    END DO
    DO k=1,n3-1
      DO j=1,n2
        DO i=1,n1
          A%u(1,1,i,j,k)=-(beta0*dtp)**2*2.0d0*F3(i,j,k+1) &
              *DTU3(i,j,k+1)*F3G(i,j,k+1)*DUT(i,j,k+1)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          A%u(1,2,i,j,k)=-(beta0*dtp)**2*2.0d0*F3(i,j,k+1) &
              *DTU3(i,j,k+1)*F3G(i,j,k+1)*DUR(i,j,k+1)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          A%u(2,1,i,j,k)=-(beta0*dtp)**2*2.0d0*F3(i,j,k+1) &
              *F3G(i,j,k+1)*DUT(i,j,k+1)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          A%u(2,2,i,j,k)=-(beta0*dtp)**2*2.0d0*F3(i,j,k+1) &
              *F3G(i,j,k+1)*DUR(i,j,k+1)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
!         A%d(k)%P%d(j)%d(i)=A%d(k)%P%d(j)%d(i)-A%e(:,i,j,k)
          ...
          A%d(k)%P%d(j)%d(1,2,i)=A%d(k)%P%d(j)%d(1,2,i) &
           +(beta0*dtp)**2*2.0d0*F3(i,j,k+1)*DTU3(i,j,k+1)*F3G(i,j,k+1)*DUR(i,j,k)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          A%d(k)%P%d(j)%d(2,1,i)=A%d(k)%P%d(j)%d(2,1,i) &
           +(beta0*dtp)**2*2.0d0*F3(i,j,k+1)*F3G(i,j,k+1)*DUT(i,j,k)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          A%d(k)%P%d(j)%d(2,2,i)=A%d(k)%P%d(j)%d(2,2,i) &
           +(beta0*dtp)**2*2.0d0*F3(i,j,k+1)*F3G(i,j,k+1)*DUR(i,j,k)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
        END DO
      END DO
      CALL Compute(A%d(k+1)%P,F1(:,:,k+1),F2(:,:,k+1),F1G(:,:,k+1),F2G(:,:,k+1),Vol(:,:,k+1), &
                   DTU1(:,:,k+1),DTU2(:,:,k+1),DUU1(:,:,k+1),DUU2(:,:,k+1),DUT(:,:,k+1),DUR(:,:,k+1))
      DO j=1,n2
        DO i=1,n1
          A%l(1,1,i,j,k)=-(beta0*dtp)**2*2.0d0*F3(i,j,k+1) &
              *DTU3(i,j,k+1)*F3G(i,j,k+1)*DUT(i,j,k)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          A%l(1,2,i,j,k)=-(beta0*dtp)**2*2.0d0*F3(i,j,k+1) &
              *DTU3(i,j,k+1)*F3G(i,j,k+1)*DUR(i,j,k)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          A%l(2,1,i,j,k)=-(beta0*dtp)**2*2.0d0*F3(i,j,k+1) &
              *F3G(i,j,k+1)*DUT(i,j,k)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          A%l(2,2,i,j,k)=-(beta0*dtp)**2*2.0d0*F3(i,j,k+1) &
              *F3G(i,j,k+1)*DUR(i,j,k)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
!         A%d(k+1)%P%d(j)%d(i)=A%d(k+1)%P%d(j)%d(i)-A%e(:,i,j,k)
          ...
          A%d(k+1)%P%d(j)%d(1,2,i)=A%d(k+1)%P%d(j)%d(1,2,i) &
           +(beta0*dtp)**2*2.0d0*F3(i,j,k+1)*DTU3(i,j,k+1)*F3G(i,j,k+1)*DUR(i,j,k+1)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          ...
          A%d(k+1)%P%d(j)%d(2,2,i)=A%d(k+1)%P%d(j)%d(2,2,i) &
           +(beta0*dtp)**2*2.0d0*F3(i,j,k+1)*F3G(i,j,k+1)*DUR(i,j,k+1)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
        END DO
      END DO
    END DO
    DO j=1,n2
      DO i=1,n1
        ...
        A%d(n3)%P%d(j)%d(1,2,i)=A%d(n3)%P%d(j)%d(1,2,i) &
           +(beta0*dtp)**2*Fac3_R*F3(i,j,n3+1)*DTU3(i,j,n3+1)*F3G(i,j,n3+1)*DUR(i,j,n3)/(Vol(i,j,n3)+Eps)
        A%d(n3)%P%d(j)%d(2,1,i)=A%d(n3)%P%d(j)%d(2,1,i) &
           +(beta0*dtp)**2*Fac3_R*F3(i,j,n3+1)*F3G(i,j,n3+1)*DUT(i,j,n3)/(Vol(i,j,n3)+Eps)
        A%d(n3)%P%d(j)%d(2,2,i)=A%d(n3)%P%d(j)%d(2,2,i) &
           +(beta0*dtp)**2*Fac3_R*F3(i,j,n3+1)*F3G(i,j,n3+1)*DUR(i,j,n3)/(Vol(i,j,n3)+Eps)
      END DO
    END DO
  ELSE
    n2=A%d(1)%P%n2
...
!         A%d(k)%P%d(j)%d(i)=A%d(k)%P%d(j)%d(i)-A%e(:,i,j,k)
          ...
          A%d(k)%P%d(j)%d(1,2,i)=A%d(k)%P%d(j)%d(1,2,i) &
           +(beta0*dtp)**2*2.0d0*F3(i,j,k+1)*DTU3(i,j,k+1)*DUU3(i,j,k+1)*F3G(i,j,k+1)*DUR(i,j,k)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          A%d(k)%P%d(j)%d(2,1,i)=A%d(k)%P%d(j)%d(2,1,i) &
           +(beta0*dtp)**2*2.0d0*F3(i,j,k+1)*DUU3(i,j,k+1)*F3G(i,j,k+1)*DUT(i,j,k)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          A%d(k)%P%d(j)%d(2,2,i)=A%d(k)%P%d(j)%d(2,2,i) &
           +(beta0*dtp)**2*2.0d0*F3(i,j,k+1)*DUU3(i,j,k+1)*F3G(i,j,k+1)*DUR(i,j,k)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
        END DO
      END DO
      CALL Compute(A%d(k+1)%P,F1(:,:,k+1),F2(:,:,k+1),F1G(:,:,k+1),F2G(:,:,k+1),Vol(:,:,k+1), &
                   DTU1(:,:,k+1),DTU2(:,:,k+1),DUU1(:,:,k+1),DUU2(:,:,k+1),DUT(:,:,k+1),DUR(:,:,k+1))
...
!         A%d(k+1)%P%d(j)%d(i)=A%d(k+1)%P%d(j)%d(i)-A%e(:,i,j,k)
          ...
          A%d(k+1)%P%d(j)%d(1,2,i)=A%d(k+1)%P%d(j)%d(1,2,i) &
           +(beta0*dtp)**2*2.0d0*F3(i,j,k+1)*DTU3(i,j,k+1)*DUU3(i,j,k+1)*F3G(i,j,k+1)*DUR(i,j,k+1)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          A%d(k+1)%P%d(j)%d(2,1,i)=A%d(k+1)%P%d(j)%d(2,1,i) &
           +(beta0*dtp)**2*2.0d0*F3(i,j,k+1)*DUU3(i,j,k+1)*F3G(i,j,k+1)*DUT(i,j,k+1)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
          A%d(k+1)%P%d(j)%d(2,2,i)=A%d(k+1)%P%d(j)%d(2,2,i) &
           +(beta0*dtp)**2*2.0d0*F3(i,j,k+1)*DUU3(i,j,k+1)*F3G(i,j,k+1)*DUR(i,j,k+1)/(Vol(i,j,k)+Vol(i,j,k+1)+Eps)
        END DO
      END DO
    END DO
    DO j=1,n2
      DO i=1,n1
        ...
        A%d(n3)%P%d(j)%d(1,2,i)=A%d(n3)%P%d(j)%d(1,2,i) &
           +(beta0*dtp)**2*Fac3_R*F3(i,j,n3+1)*DTU3(i,j,n3+1)*DUU3(i,j,n3+1)*F3G(i,j,n3+1)*DUR(i,j,n3)/(Vol(i,j,n3)+Eps)
        A%d(n3)%P%d(j)%d(2,1,i)=A%d(n3)%P%d(j)%d(2,1,i) &
           +(beta0*dtp)**2*Fac3_R*F3(i,j,n3+1)*DUU3(i,j,n3+1)*F3G(i,j,n3+1)*DUT(i,j,n3)/(Vol(i,j,n3)+Eps)
        A%d(n3)%P%d(j)%d(2,2,i)=A%d(n3)%P%d(j)%d(2,2,i) &
           +(beta0*dtp)**2*Fac3_R*F3(i,j,n3+1)*DUU3(i,j,n3+1)*F3G(i,j,n3+1)*DUR(i,j,n3)/(Vol(i,j,n3)+Eps)
      END DO
    END DO
  END IF
...
SUBROUTINE MatVecTR_3D(y,A,x)
  ...
  n2=A%n2
  n3=A%n3
  CALL MatVecTR(y(:,:,:,1),A%d(1)%P,x(:,:,:,1))
  IF (n3>1) THEN
    DO j=1,n2
      DO i=1,n1
        y(1:2,i,j,1)=y(1:2,i,j,1)+Mult_2(A%u(1:2,1:2,i,j,1),x(1:2,i,j,2))
      END DO   
    END DO   
    DO k=2,n3-1
      CALL MatVecTR(y(:,:,:,k),A%d(k)%P,x(:,:,:,k))
      DO j=1,n2
        DO i=1,n1
          y(1:2,i,j,k)=y(1:2,i,j,k)+Mult_2(A%l(1:2,1:2,i,j,k-1),x(1:2,i,j,k-1))+Mult_2(A%u(1:2,1:2,i,j,k),x(1:2,i,j,k+1))
        END DO   
      END DO   
    END DO
    CALL MatVecTR(y(:,:,:,n3),A%d(n3)%P,x(:,:,:,n3))
    DO j=1,n2
      DO i=1,n1
        y(1:2,i,j,n3)=y(1:2,i,j,n3)+Mult_2(A%l(1:2,1:2,i,j,n3-1),x(1:2,i,j,n3-1))
      END DO   
    END DO   
  END IF

END SUBROUTINE MatVecTR_3D
  ...
  IF (A%n3>1) THEN
    DO iter=1,GaussIter
      ...
      DO k=1,A%n3-1
        DO j=1,A%n2
          DO i=1,A%n1
            t(1:2,i,j)=t(1:2,i,j)-Mult_2(A%u(1:2,1:2,i,j,k),x(1:2,i,j,k+1))
          END DO   
        END DO   
        CALL Solve(A%d(k)%P,t,x(:,:,:,k))
        DO j=1,A%n2
          DO i=1,A%n1
            t(1:2,i,j)=b(1:2,i,j,k+1)-Mult_2(A%l(1:2,1:2,i,j,k),x(1:2,i,j,k))
          END DO   
        END DO   
      END DO
      ...
      DO j=1,A%n2
        DO i=1,A%n1
          t(1:2,i,j)=b(1:2,i,j,A%n3-1)-Mult_2(A%u(1:2,1:2,i,j,A%n3-1),x(1:2,i,j,A%n3))
        END DO   
      END DO   
      DO k=A%n3-1,2,-1
        DO j=1,A%n2
          DO i=1,A%n1
            t(1:2,i,j)=t(1:2,i,j)-Mult_2(A%l(1:2,1:2,i,j,k-1),x(1:2,i,j,k-1))
          END DO   
        END DO   
        CALL Solve(A%d(k)%P,t,x(:,:,:,k))
        DO j=1,A%n2
          DO i=1,A%n1
            t(1:2,i,j)=b(1:2,i,j,k-1)-Mult_2(A%u(1:2,1:2,i,j,k-1),x(1:2,i,j,k))
          END DO   
        END DO   
      END DO
    END DO
...
!   zusaetzlich 
    DO j=1,A%n2
      DO i=1,A%n1
        t(1:2,i,j)=b(1:2,i,j,A%n3)-Mult_2(A%l(1:2,1:2,i,j,A%n3-1),x(1:2,i,j,A%n3-1))
      END DO
    END DO
    CALL Solve(A%d(A%n3)%P,t,x(:,:,:,A%n3))
!   zusaetzlich 
...
SUBROUTINE Multigrid_3D(A,b,x)
  ...
  TYPE(TriTRDiag3D_T), TARGET :: A
  REAL(RealKind), TARGET :: b(:,:,:,:)
  REAL(RealKind), TARGET :: x(:,:,:,:)

  INTEGER :: i,j,k,n1,n2,n3
  TYPE(TriTRDiag3D_T), POINTER :: ALoc
  REAL(RealKind), POINTER :: bLoc(:,:,:,:)
  REAL(RealKind), POINTER :: xLoc(:,:,:,:)
  INTEGER :: GaussIter
  REAL(RealKind) :: Res(SIZE(b,1),SIZE(b,2),SIZE(b,3),SIZE(b,4))
...
SUBROUTINE RestrictTR_3D(Res,ResCoarse)
  ...
  DO kC=1,n3/2
    ...
    kF=kF+2
  END DO
END SUBROUTINE RestrictTR_3D

SUBROUTINE ProlongateTR_3D(xCoarse,x)
...
END SUBROUTINE ProlongateTR_3D
END MODULE TriTR_3D_Mod
