...
!  nc_err = nf90_get_var(ncid, var_btype_id, btype)
  ...
  DO n = 1, SIZE(subdom%blockids_comp)
    ...
    DO m = 1, SIZE(neighbors)
       is_ghost(neighbors(m)) = .TRUE.
    END DO
  END DO
 ...
 ! Get the scaling fields
  ...
  CALL check_common_edges(subdom%blocks)
  CALL check_common_faces(subdom%blocks)

END SUBROUTINE read_blocks_from_netcdf


SUBROUTINE check_common_edges(blocks)
...
SUBROUTINE check_common_faces(blocks)
