...
SUBROUTINE grid_blocks(finest_grid, block_ispartof, block_npartof, x2_glob, y2_glob, z2_glob, &
  ...
  ALLOCATE(block_ispartof(nblocks, 100, 3))
  ...
  ngrids_x = INT(LOG(REAL(finest_grid%nx)) / LOG(2.0))
  ngrids_y = INT(LOG(REAL(finest_grid%ny)) / LOG(2.0))
  ngrids_z = INT(LOG(REAL(finest_grid%nz)) / LOG(2.0))
  ...
  DO igrid = 1, ngrids_coarse
    ...
    grids_coarse(igrid)%nx = MAX(finest_grid%nx / 2 ** igrid, 1)
    grids_coarse(igrid)%ny = MAX(finest_grid%ny / 2 ** igrid, 1) 
    grids_coarse(igrid)%nz = MAX(finest_grid%nz / 2 ** igrid, 1)

    ALLOCATE(grids_coarse(igrid)%cells(grids_coarse(igrid)%nz, grids_coarse(igrid)%ny, grids_coarse(igrid)%nx))

    ALLOCATE(grids_coarse(igrid)%ix2(grids_coarse(igrid)%nx + 1))
    ALLOCATE(grids_coarse(igrid)%jy2(grids_coarse(igrid)%ny + 1))
    ALLOCATE(grids_coarse(igrid)%kz2(grids_coarse(igrid)%nz + 1))

    DO i = 1, grids_coarse(igrid)%nx + 1
      grids_coarse(igrid)%ix2(i) = finest_grid%ix2(MIN(2 ** igrid * (i - 1) + 1, finest_grid%nx + 1))
    END DO
    grids_coarse(igrid)%ix2(grids_coarse(igrid)%nx + 1) = finest_grid%ix2(finest_grid%nx + 1)

    DO j = 1, grids_coarse(igrid)%ny + 1
      grids_coarse(igrid)%jy2(j) = finest_grid%jy2(MIN(2 ** igrid * (j - 1) + 1, finest_grid%ny + 1))
    END DO
    grids_coarse(igrid)%jy2(grids_coarse(igrid)%ny + 1) = finest_grid%jy2(finest_grid%ny + 1)

    DO k = 1, grids_coarse(igrid)%nz + 1
      grids_coarse(igrid)%kz2(k) = finest_grid%kz2(MIN(2 ** igrid * (k - 1) + 1, finest_grid%nz + 1))
    END DO
    grids_coarse(igrid)%kz2(grids_coarse(igrid)%nz + 1) = finest_grid%kz2(finest_grid%nz + 1)
  ...
  IF (ngrids_coarse .GT. 0) THEN
    ...
    DO igrid = 1, ngrids_coarse - 1
      CALL connect_grids(grids_coarse(i), grids_coarse(i + 1))
    END DO
  ...
  DO i = 1, grid_f%nx
    DO j = 1, grid_f%ny
      DO k = 1, grid_f%nz
        ...
        ind_x = 0
        DO ii = 1, grid_c%nx
          dist_new = ABS(x_val - 0.5 * REAL(grid_c%ix2(ii) + grid_c%ix2(ii + 1)))
        ...
        ind_y = 0
        DO jj = 1, grid_c%ny
          dist_new = ABS(y_val - 0.5 * REAL(grid_c%jy2(jj) + grid_c%jy2(jj + 1)))
          IF (dist_new < dist) THEN
            ind_y = ind_y + 1
            dist = dist_new
          ELSE
        ...
        ind_z = 0
        DO kk = 1, grid_c%nz
          dist_new = ABS(z_val - 0.5 * REAL(grid_c%kz2(kk) + grid_c%kz2(kk + 1)))
          IF (dist_new < dist) THEN
            ind_z = ind_z + 1
            dist = dist_new
          ELSE
...
FUNCTION neighborblocks(finest_grid, block_ispartof, block_npartof, thisblock, &
                       i_glob_st, i_glob_end, j_glob_st, j_glob_end, k_glob_st, k_glob_end) 
  IMPLICIT NONE
  ...
  INTEGER :: ineighbors_tmp(100), ineigh
  INTEGER, POINTER :: neighborblocks(:)
  INTEGER, INTENT(in) :: i_glob_st(:), i_glob_end(:), &
                         j_glob_st(:), j_glob_end(:), &
                         k_glob_st(:), k_glob_end(:)
  ...
  DO icell = 1, npartof
    ...
    DO ind = 1, nblocks_within
      ...
      IF (.NOT. iblock .EQ. thisblock) THEN
        IF (i_glob_st(thisblock) .EQ. i_glob_end(iblock) + 1) THEN
          IF (.NOT. ANY((/(j_glob_st(thisblock) .LT. j_glob_st(iblock)) .AND. &
                       ...
                       /))) THEN 
            ! bounds to the west
            IF (.NOT. isin(ineighbors_tmp(1:ineigh - 1), iblock)) THEN
              ineighbors_tmp(ineigh) = iblock
              ineigh = ineigh + 1
            END IF
          END IF
        END IF
        IF (i_glob_end(thisblock) + 1 .EQ. i_glob_st(iblock)) THEN
          IF (.NOT. ANY((/(j_glob_st(thisblock) .LT. j_glob_st(iblock)) .AND. &
                          (j_glob_end(thisblock) + 1 .LT. j_glob_st(iblock)), &
                          (j_glob_st(thisblock) .GT. j_glob_end(iblock) + 1) .AND. &
                          (j_glob_end(thisblock) + 1 .GT. j_glob_end(iblock) + 1), & 
                          (k_glob_st(thisblock) .LT. k_glob_st(iblock)) .AND. &
                          (k_glob_end(thisblock) + 1 .LT. k_glob_st(iblock)), &
                          (k_glob_st(thisblock) .GT. k_glob_end(iblock) + 1) .AND. &
                          (k_glob_end(thisblock) + 1 .GT. k_glob_end(iblock) + 1) &
                       /))) THEN
            ! bounds to the east
            IF (.NOT. isin(ineighbors_tmp(1:ineigh - 1), iblock)) THEN
              ineighbors_tmp(ineigh) = iblock
          ...
          END IF
        END IF
        IF (j_glob_st(thisblock) .EQ. j_glob_end(iblock) + 1) THEN
          IF (.NOT. ANY((/(i_glob_st(thisblock) .LT. i_glob_st(iblock)) .AND. &
                          ...
                          (i_glob_end(thisblock) + 1 .GT. i_glob_end(iblock) + 1), & 
                          (k_glob_st(thisblock) .LT. k_glob_st(iblock)) .AND. &
                          (k_glob_end(thisblock) + 1 .LT. k_glob_st(iblock)), &
                          (k_glob_st(thisblock) .GT. k_glob_end(iblock) + 1) .AND. &
                          (k_glob_end(thisblock) + 1 .GT. k_glob_end(iblock) + 1) &
          ...
          END IF
        END IF
        IF (j_glob_end(thisblock) + 1 .EQ. j_glob_st(iblock)) THEN
          IF (.NOT. ANY((/(i_glob_st(thisblock) .LT. i_glob_st(iblock)) .AND. &
                          (i_glob_end(thisblock) + 1 .LT. i_glob_st(iblock)), &
                          (i_glob_st(thisblock) .GT. i_glob_end(iblock) + 1) .AND. &
                          (i_glob_end(thisblock) + 1 .GT. i_glob_end(iblock) + 1), &
          ...
          END IF
        END IF
        IF (k_glob_st(thisblock) .EQ. k_glob_end(iblock) + 1) THEN
          IF (.NOT. ANY((/(i_glob_st(thisblock) .LT. i_glob_st(iblock)) .AND. &
                          (i_glob_end(thisblock) + 1 .LT. i_glob_st(iblock)), &
                          (i_glob_st(thisblock) .GT. i_glob_end(iblock) + 1) .AND. &
                          (i_glob_end(thisblock) + 1 .GT. i_glob_end(iblock) + 1), &
          ...
          END IF
        END IF
        IF (k_glob_end(thisblock) + 1 .EQ. k_glob_st(iblock)) THEN
          IF (.NOT. ANY((/(i_glob_st(thisblock) .LT. i_glob_st(iblock)) .AND. &
                          (i_glob_end(thisblock) + 1 .LT. i_glob_st(iblock)), &
                          (i_glob_st(thisblock) .GT. i_glob_end(iblock) + 1) .AND. &
                          (i_glob_end(thisblock) + 1 .GT. i_glob_end(iblock) + 1), &
  ...
  ALLOCATE(neighborblocks(ineigh - 1))
  neighborblocks(:) = ineighbors_tmp(1:ineigh - 1)

END FUNCTION neighborblocks
