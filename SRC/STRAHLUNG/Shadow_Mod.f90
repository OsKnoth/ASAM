   ...
   DO ibLoc=1,nbLoc              ! follow rays
     ...
     IF(L(ibLoc)%n>0)THEN
       DO i=1,L(ibLoc)%n
         ...
         iy=r%iy
         iz=r%iz
        !WRITE(*,*) MyID,ib,LBOUND(dx,1),UBOUND(dx,1),ix,iy,iz
         step=0.01d0*MIN(dx(ix0+1),dy(iy0+1),dz(iz0+1))
...
!              IF (ixb/=r%ix0.AND.iyb/=r%iy0.AND.izb/=r%iz0) THEN
               IF (ixb/=r%ix0.OR.iyb/=r%iy0.OR.izb/=r%iz0) THEN
...
!              Aussenrand oder Höhe 
 ...
 SUBROUTINE ExchangeShadow(L)
   ...
   INTEGER :: ireq,tag
   TYPE(MPI_Request) :: req(2*nbLoc*nb)
   TYPE(MPI_Status) :: StatusArray(2*nb*nb)
   TYPE(RayT),POINTER :: r
   TYPE(RayT) :: rT
