...
! #Baum:
  ...
  CHARACTER(64) :: file_landclass_utm
  INTEGER, ALLOCATABLE :: LandCover(:,:)
  INTEGER, ALLOCATABLE :: LandCoverXCoord(:,:)
  INTEGER, ALLOCATABLE :: LandCoverYCoord(:,:)
  TYPE rgb_T
    INTEGER :: nr_r
    INTEGER :: nr_g
    INTEGER :: nr_b
...
!     Level=f_bispev(x,y,z)
    ...
    CASE ('UTM1')
      Level=f_bispev(x,y,z)
    CASE ('Haus','Haus1') 
     Level=DistanceHaus(x,y,z,DistSurf)
    CASE ('HalfCircle')
    ...
    CASE ('Agnesi2DP')
      ...
      END IF
    CASE ('Agnesi3D')
      Level=z-H/(1.0d0+((x-xM)/aX)**2+((y-yM)/aY)**2)
    CASE ('Bell')
...
!           x4=(B+D)/2+-SQRT(1/4*(B*D)^2-BD-C/A)
        ...
        CASE ('Flat')
          READ(InputUnit,*) FlatShift
        CASE ('Haus')  
          READ(InputUnit,*) FileData
          READ(InputUnit,*) DistSurf
        ...
        CASE ('Aster')  
          READ(InputUnit,*) NS0,NS1
          READ(InputUnit,*) EW0,EW1
          CALL ReadAster
        CASE ('WGS84Surf')  
          READ(InputUnit,*) FileData
...
SUBROUTINE  ReadHaus(FileName)
  ...
  DO h=1,NumberHaus
    ...
    Haus(h)%NumberOfPoints=0
    DO i=1,Haus(h)%NumberOfFaces
      Haus(h)%NumberOfPoints=Haus(h)%NumberOfPoints+Haus(h)%Faces(i)%NumberOfPoints
      ALLOCATE(Haus(h)%Faces(i)%Points(Haus(h)%Faces(i)%NumberOfPoints))
      ALLOCATE(Haus(h)%Faces(i)%ListOfPoints(Haus(h)%Faces(i)%NumberOfPoints))
    ...
    DO i=1,Haus(h)%NumberOfFaces
      DO j=1,Haus(h)%Faces(i)%NumberOfPoints
        l=l+1
        Haus(h)%Points(l)=Haus(h)%Faces(i)%Points(j)
        Haus(h)%Faces(i)%ListOfPoints(j)=l
      END DO
    END DO  
  ...
  ALLOCATE(Root)
  NULLIFY(Root%List)
  Root%P0%x=MIN(MAX(Domain%x0,x0DomainHaus),x0Tree)
  Root%P0%y=MIN(MAX(Domain%y0,y0DomainHaus),y0Tree)
  Root%P0%z=Domain%z0
  Root%P1%x=MAX(MIN(Domain%x1,x1DomainHaus),x1Tree)
  Root%P1%y=MAX(MIN(Domain%y1,y1DomainHaus),y1Tree)
  Root%P1%z=Domain%z1
  Root%TypeCut='x'
  Root%Index=0
  Root%Depth=Depth
...
SUBROUTINE  ReadHaus1(FileName)
  ...
  ALLOCATE(Haus(1:NumberHaus))
  DO h=1,NumberHaus
    READ(InputUnitData,*) Haus(h)%Number
    READ(InputUnitData,*) Haus(h)%NumberOfPoints
    ALLOCATE(Haus(h)%Points(Haus(h)%NumberOfPoints))
    DO i=1,Haus(h)%NumberOfPoints
      READ(InputUnitData,*) Haus(h)%Points(i)
    END DO  
    READ(InputUnitData,*) Haus(h)%NumberOfFaces
    ALLOCATE(Haus(h)%Faces(Haus(h)%NumberOfFaces))
    READ(InputUnitData,*) (Haus(h)%Faces(i)%NumberOfPoints, i=1,Haus(h)%NumberOfFaces)
    READ(InputUnitData,*) (Haus(h)%Faces(i)%type, i=1,Haus(h)%NumberOfFaces)
    DO i=1,Haus(h)%NumberOfFaces
      ALLOCATE(Haus(h)%Faces(i)%Points(Haus(h)%Faces(i)%NumberOfPoints))
      ALLOCATE(Haus(h)%Faces(i)%ListOfPoints(Haus(h)%Faces(i)%NumberOfPoints))
      READ(InputUnitData,*) (Haus(h)%Faces(i)%ListOfPoints(j),j=1,Haus(h)%Faces(i)%NumberOfPoints) 
      DO j=1,Haus(h)%Faces(i)%NumberOfPoints
        Haus(h)%Faces(i)%Points(j)=Haus(h)%Points(Haus(h)%Faces(i)%ListOfPoints(j))
      END DO  
    END DO
    CALL NormalForm(Haus(h))
    CALL BoundingBox(Haus(h))
...
! END IF  
  Root%P0%x=MIN(MAX(Domain%x0,x0DomainHaus),x0Tree)
  Root%P0%y=MIN(MAX(Domain%y0,y0DomainHaus),y0Tree)
  WRITE(*,*) 'Root%P0%y ',Domain%y0,y0DomainHaus,y0Tree
  Root%P0%z=MIN(Domain%z0,z0Tree)
  Root%P1%x=MAX(MIN(Domain%x1,x1DomainHaus),x1Tree)
  Root%P1%y=MAX(MIN(Domain%y1,y1DomainHaus),y1Tree)
  Root%P1%z=MAX(Domain%z1,z1Tree)
  WRITE(*,*) 'Root%P0',Root%P0
  WRITE(*,*) 'Root%P1',Root%P1
  Root%TypeCut='x'
  Root%Index=0
  Root%Depth=Depth
  CALL CreateTree(Root,Depth)
  DO i=1,NumberHaus
    IF (InsideBoxHaus(Root,Haus(i))) THEN
      WRITE(*,*) 'Haus will be inserted',i
      CALL InsertHaus(Root,Haus(i),i)
    END IF  
  END DO
END SUBROUTINE ReadHaus1
...
SUBROUTINE ReadUTM(FileName)
  ...
  READ(InputUnitData,*) dxH
  READ(InputUnitData,*) dyH
  READ(InputUnitData,*) DummyRow,DummyRow,DummyRow
  ALLOCATE(xPH(nxH))
  ALLOCATE(yPH(nyH))
  ALLOCATE(Height(nxH,nyH))
  nrows=nxH*nyH
...
! yPH(1)=0.0
  ...
  WRITE(*,*) 'Smoothing data (smoothing factor =',SFac,', iterations: ',NumSmooth,')'
  ALLOCATE(Work(nxH,nyH))
  WRITE(*,*) 'MaxHeight, MinHeight',MAXVAL(Height),MINVAL(Height)
  DO iSmooth=1,NumSmooth
    Work=Height
    DO j=2,nyH-1
      DO i=2,nxH-1
...
SUBROUTINE ReadAster

  INTEGER :: iNS,iEW
  INTEGER :: iLat,iLon
  INTEGER :: nLat,nLon
  INTEGER :: nLatLoc,nLonLoc
  INTEGER :: iLatNeu,iLonNeu
  INTEGER :: nLatNeu,nLonNeu
  INTEGER :: iSmooth
  REAL(8) :: SFac
  REAL(8) :: Lat0,Lon0,dL,Dummy
  REAL(8) :: Lat0Loc,Lon0Loc
  CHARACTER(20) :: FileName
  REAL(8), ALLOCATABLE :: Work(:,:)
  INTEGER :: iCoarse
  REAL(8), ALLOCATABLE :: HeightNeu(:,:)
  ...
  IF (ALLOCATED(Height)) THEN
    RETURN
  END IF
  nLon=(EW1-EW0+1)*3600+1
  nLat=(NS1-NS0+1)*3600+1
  WRITE(*,*) 'nLon ',EW1,EW0,nLon 
  WRITE(*,*) 'nLat ',NS1,NS0,nLat 
  ALLOCATE(Height(nLon,nLat))
  ...
  DO iNS=NS0,NS1
    DO iEW=EW0,EW1
      ...
      OPEN(UNIT=InputUnitData,FILE=TRIM(FileName)//'.asc',STATUS='OLD')
      READ(InputUnitData,*) def_str,nLonLoc
      READ(InputUnitData,*) def_str,nLatLoc
      READ(InputUnitData,*) def_str,Lon0Loc
      READ(InputUnitData,*) def_str,Lat0Loc
      READ(InputUnitData,*) def_str,dL
      READ(InputUnitData,*) def_str,Dummy
      IF (iNS==NS0.AND.iEW==EW0) THEN
        Lon0=Lon0Loc
        Lat0=Lat0Loc
      END IF  
      DO iLat=3601,1,-1
        READ(InputUnitData,*) (Height(iLon+(iEW-EW0)*3600,iLat+(iNS-NS0)*3600),iLon=1,3601)
      END DO
      CLOSE(InputUnitData)
      WRITE(*,*) 'Height ',MAXVAL(Height),MINVAL(Height)
    END DO  
  END DO  
  Height=MAX(Height,0.0d0)
! Coarse data
  WRITE(*,*) 'NumCoarse ',NumCoarse
  DO iCoarse=1,NumCoarse
    nLonNeu=(nLon-1)/2+1
    nLatNeu=(nLat-1)/2+1
    WRITE(*,*) 'Coarse nLon',nLon,nLonNeu
    WRITE(*,*) 'Coarse nLat',nLat,nLatNeu
    ALLOCATE(HeightNeu(nLonNeu,nLatNeu))
    HeightNeu=0.0d0
    DO iLonNeu=1,nLonNeu
      DO iLatNeu=1,nLatNeu
        iLon=MIN(MAX(2*iLonNeu-1,1),nLon)
        iLat=MIN(MAX(2*iLatNeu-1,1),nLat)
        HeightNeu(iLonNeu,iLatNeu)=HeightNeu(iLonNeu,iLatNeu)+Height(iLon,iLat)
        iLon=MIN(MAX(2*iLonNeu  ,1),nLon)
        iLat=MIN(MAX(2*iLatNeu-1,1),nLat)
        HeightNeu(iLonNeu,iLatNeu)=HeightNeu(iLonNeu,iLatNeu)+Height(iLon,iLat)
        iLon=MIN(MAX(2*iLonNeu+1,1),nLon)
        iLat=MIN(MAX(2*iLatNeu-1,1),nLat)
        ...
        iLat=MIN(MAX(2*iLatNeu+1,1),nLat)
        HeightNeu(iLonNeu,iLatNeu)=HeightNeu(iLonNeu,iLatNeu)+Height(iLon,iLat)
        HeightNeu(iLonNeu,iLatNeu)=HeightNeu(iLonNeu,iLatNeu)/9.0d0
      END DO
    END DO
    DEALLOCATE(Height)
    nLat=nLatNeu
    ...
    ALLOCATE(Height(nLon,nLat))
    Height=HeightNeu
    DEALLOCATE(HeightNeu)
    dL=2*dL
  END DO
  WRITE(*,*) 'Coarse Ende nLon',nLon
  WRITE(*,*) 'Coarse Ende nLat',nLat

  y0H=(Lat0*4.0d0*ATAN(1.0d0))/180.0d0
  x0H=(Lon0*4.0d0*ATAN(1.0d0))/180.0d0
  dL=(dL*4.0d0*ATAN(1.0d0))/180.0d0
  WRITE(*,*) 'x0H',x0H,Lon0
  WRITE(*,*) 'y0H',y0H,Lat0
  ALLOCATE(xPH(nLon))
  ALLOCATE(yPH(nLat))
  xPH(1)=x0H
  DO iLon=2,nLon
     xPH(iLon)=xPH(iLon-1)+dL
  END DO
  yPH(1)=y0H
  DO iLat=2,nLat
     yPH(iLat)=yPH(iLat-1)+dL
  END DO
  nxH=nLon
  nyH=nLat
  dxH=dL
  ...
  DO iSmooth=1,NumSmooth
    ...
    DO iLat=2,nyH-1
      DO iLon=2,nxH-1
        Height(iLon,iLat)=Work(iLon,iLat)+SFac*(Work(iLon+1,iLat)+Work(iLon-1,iLat)+Work(iLon,iLat+1)+Work(iLon,iLat-1))
        Height(iLon,iLat)=Height(iLon,iLat)/(1.0d0+4.0d0*SFac)
      END DO  
    END DO  
    WRITE(*,*) 'MaxHeight, MinHeight',MAXVAL(Height),MINVAL(Height)
  END DO  
...
SUBROUTINE Read_LandClass_UTM
  ...
  CHARACTER (LEN=20)  :: nroutine     ! name of this subroutine
  CHARACTER (LEN=40)  :: nerrmsg      ! error message
  INTEGER             :: nstat,cstat,cstatx,cstaty  ! error status variables
  INTEGER             :: dummy  ! error status variables
  INTEGER             :: ix,iy
  INTEGER             :: nx,ny,x0,y0

  IF (ALLOCATED(LandCover)) THEN
    WRITE(*,*) 'Routine Read_LandClass_UTM: LandCover already allocated! Return.'
    RETURN
  END IF
  OPEN(UNIT=InUnit,FILE=TRIM(file_landclass_utm),STATUS='OLD',IOSTAT=nstat)
  IF(nstat/=0) THEN
     nroutine='Read_LandClass_UTM'
     nerrmsg = "OPENING OF FILE \'file_landclass_utm\' FAILED"
     Write(*,*) nerrmsg
     !CALL tool_break (my_cart_id, 1001, nerrmsg, nroutine)
  ELSE 
    READ(InUnit,*) read_line ! comment
    READ(InUnit,*) dxUTM,dyUTM    
    READ(InUnit,*) nx,ny
    READ(InUnit,*) x0,y0
    READ(InUnit,*) read_line ! comment

    ALLOCATE(LandCover(1:nx,1:ny),STAT=cstat)
    ALLOCATE(LandCoverXCoord(1:nx,1:ny),STAT=cstatx)
    ALLOCATE(LandCoverYCoord(1:nx,1:ny),STAT=cstaty)
    DO ix=1,nx
      DO iy=1,ny
        READ(InUnit,*) dummy,dummy,dummy,LandCover(ix,iy)
        LandCoverXCoord(ix,iy)=x0+(ix-1)*dxUTM
        LandCoverYCoord(ix,iy)=y0+(iy-1)*dyUTM
      END DO
    END DO   
    CLOSE(UNIT=InUnit)       
...
!   DO ix=1,nx
!     DO iy=1,ny    
!       WRITE(*,*) 'LandCover',ix,iy,LandCover(ix,iy),LandCoverXCoord(ix,iy),LandCoverYCoord(ix,iy)
!     END DO
!   END DO
  END IF

END SUBROUTINE Read_LandClass_UTM
...
FUNCTION DistanceHaus(x,y,z,DistSurf)
  REAL(8) :: DistanceHaus
  REAL(8) :: x,y,z
  REAL(8):: DistSurf
  INTEGER :: i,h
  ...
  S2:DO
    IF (ASSOCIATED(Box)) THEN
      IF (ASSOCIATED(Box%List)) THEN
        S1:DO i=1,SIZE(Box%List)
          disp=Dist(Pdis,Haus(Box%List(i)),DistSurf)
          iF=iF+1
          dispF(iF)=disp
          DistanceHaus=MIN(DistanceHaus,disp)
          IF (DistanceHaus<-1.d-5) EXIT S2
        END DO S1
      END IF
      Box=>Box%Parent
    ELSE
...
FUNCTION BiQuadratic(x,y,z)
  ...
  IF (iPos<0) THEN
    WRITE(*,*) 'iPos<0',x,x0H,iPos
  END IF  
  IF (iPos+1>SIZE(xPH)) THEN
    WRITE(*,*) 'iPos>SIZE(xPH)',x,x0H,iPos,SIZE(xPH),xPH(1),xPH(SIZE(xPH))
  END IF  
  IF (jPos<0) THEN
    WRITE(*,*) 'jPos<0',y,y0H,jPos
  END IF  
  IF (jPos+1>SIZE(yPH)) THEN
    WRITE(*,*) 'jPos>SIZE(yPH)',y,y0H,jPos,SIZE(yPH)
  END IF  
  x1=xPH(iPos)
  x2=xPH(iPos+1)
  y1=yPH(jPos)
