    ...
    IF ((Cell%in_out>-8.AND.Cell%Vol>0.0d0) &
      ...
      IF (Cell%vc==0) THEN
       ! hex
        ...
        WRITE(10,'(8I8)') (VertViewScale(Cell%Face2%VertexList(l1)),l1=1,4) &
                         ,(VertViewScale(Cell%Face1%VertexList(l2)),l2=1,4)
      ELSE
        ! general
        nFaces=1
       ...
       ! WRITE(10,*) "Cell(i,j,k)", i, j, k," ASSOCIATED(Cell),vc>0"
        ...
        !END IF
        IF (Cell%Face1%NumberVert>2) THEN
          WRITE(10,*) (VertViewScale(Cell%Face1%VertexList(l1)), &
                       l1=1,Cell%Face1%NumberVert)
        END IF
        IF (Cell%Face2%NumberVert>2) THEN
          WRITE(10,*) (VertViewScale(Cell%Face2%VertexList(l1)), &
                       l1=1,Cell%Face2%NumberVert)
        END IF
        IF (Cell%Face3%NumberVert>2) THEN
          WRITE(10,*) (VertViewScale(Cell%Face3%VertexList(l1)), &
                       l1=1,Cell%Face3%NumberVert)
        END IF
        IF (Cell%Face4%NumberVert>2) THEN
          WRITE(10,*) (VertViewScale(Cell%Face4%VertexList(l1)), &
                       l1=1,Cell%Face4%NumberVert)
        END IF
        IF (Cell%Face5%NumberVert>2) THEN
          WRITE(10,*) (VertViewScale(Cell%Face5%VertexList(l1)), &
                       l1=1,Cell%Face5%NumberVert)
        END IF
        IF (Cell%Face6%NumberVert>2) THEN
          WRITE(10,*) (VertViewScale(Cell%Face6%VertexList(l1)), &
                       l1=1,Cell%Face6%NumberVert)
        END IF
        WRITE(10,*) (VertViewScale(Cell%VertCut(l1)), &
                     l1=1,Cell%vc)
      END IF
    END IF
  ELSE !!! .NOT. ASSOCIATED(Cell)
    ...
    IF (in_out>=0) THEN
     ...
     !        5--------6
      ...
      WRITE(10,'(a8,I8)') hex,8
      WRITE(10,'(8I8)')  &
           VertViewScale(Vertices(i-1,j-1,k)%nrP) &
          ,VertViewScale(Vertices(i,j-1,k)%nrP)   &
          ,VertViewScale(Vertices(i,j,k)%nrP)   &
          ,VertViewScale(Vertices(i-1,j,k)%nrP)   &
          ,VertViewScale(Vertices(i-1,j-1,k-1)%nrP)     &
          ,VertViewScale(Vertices(i,j-1,k-1)%nrP)     &
          ,VertViewScale(Vertices(i,j,k-1)%nrP)     &
          ,VertViewScale(Vertices(i-1,j,k-1)%nrP)
    END IF
  END IF
END SUBROUTINE WriteCellGMVAscii

SUBROUTINE WriteCutFacePolyGMVAscii(Face1,matv)
  ...
  ALLOCATE(zc(1:nvert))
  DO li=1,nvert
    xc(li)=xParametricOut(VertOut(Face1%VertexList(li)))
    yc(li)=yParametricOut(VertOut(Face1%VertexList(li)))
    zc(li)=zParametricOut(VertOut(Face1%VertexList(li)))
  END DO
  Write(10,*) matv, nvert &
      ,(xc(li),li=1,nvert) &
      ,(yc(li),li=1,nvert) &
  ...
  DEALLOCATE(yc)
  DEALLOCATE(zc)
END SUBROUTINE WriteCutFacePolyGMVAscii
      ...
      IF(k==1) THEN
        ...
        IF(gl/=Cell%vc) THEN
          IF((Cell%Face1%Edge1%yes_sp==1.OR.Cell%Face1%Edge2%yes_sp==1.OR. &
             ...
             ! --------
             CALL WriteCutFacePolyGMVAscii(Cell%Face1,mat2)

          ELSE IF(Cell%Face1%in_out==-1) THEN  
             ! polygon: (Face1-3Points)
             ! --------
             CALL WriteCutFacePolyGMVAscii(Cell%Face1,mat3)
          END IF
      ...
      IF(k==1) THEN
        ...
        IF(gl/=Cell%vc) THEN
          IF(Cell%Face1%Edge1%yes_sp==1.OR.Cell%Face1%Edge2%yes_sp==1.OR. &
             ...
             nFaces=1
             nVerts(1)=Cell%Face1%NumberVert
  ...
  ELSE
    ...
    IF (in_out>=0) THEN
     ! hex
      ...
      WRITE(10,REC=nRec) 8
      nRec=nRec+1
      WRITE(10,REC=nRec) VertViewScale(Vertices(i-1,j-1,k)%nrP)
      nRec=nRec+1
      WRITE(10,REC=nRec) VertViewScale(Vertices(i,j-1,k)%nrP)
      nRec=nRec+1
      WRITE(10,REC=nRec) VertViewScale(Vertices(i,j,k)%nrP)
      nRec=nRec+1
      WRITE(10,REC=nRec) VertViewScale(Vertices(i-1,j,k)%nrP)
      nRec=nRec+1
      WRITE(10,REC=nRec) VertViewScale(Vertices(i-1,j-1,k-1)%nrP)
      nRec=nRec+1
      WRITE(10,REC=nRec) VertViewScale(Vertices(i,j-1,k-1)%nrP)
      nRec=nRec+1
      WRITE(10,REC=nRec) VertViewScale(Vertices(i,j,k-1)%nrP)
      nRec=nRec+1
      WRITE(10,REC=nRec) VertViewScale(Vertices(i-1,j,k-1)%nrP)
    END IF
  END IF
...
SUBROUTINE WriteCutFacePolyGMVBinary(Face1,matv)
  ...
  DO li=1,nvert
    nRec=nRec+1
    tempw=xParametricOut(VertOut(Face1%VertexList(li)))
    WRITE(10,REC=nRec) tempw
  END DO
  DO li=1,nvert
    nRec=nRec+1
    tempw=yParametricOut(VertOut(Face1%VertexList(li)))
    WRITE(10,REC=nRec) tempw
  END DO
  DO li=1,nvert
    nRec=nRec+1
    tempw=zParametricOut(VertOut(Face1%VertexList(li)))
    WRITE(10,REC=nRec) tempw
  END DO
...
SUBROUTINE WriteAllCellsAsciiGMV(FileName)
  ...
  WRITE(10,'(a8,i8)') nodes,nr_view_out
  WRITE(10,*) (xParametricOut(VertOutView(i)),i=1,nr_view_out)
  WRITE(10,*) (yParametricOut(VertOutView(i)),i=1,nr_view_out)
  WRITE(10,*) (zParametricOut(VertOutView(i)),i=1,nr_view_out)
  ...
  ! Soil-layering:
  !-----------
  !Write(*,*) "WritePolySoilLayerGMVAsciiBin"
             ...
             DO k=v_z0+1,v_z1
               DO j=v_y0+1,v_y1
                 DO i=v_x0+1,v_x1
                     ...
                     CALL WritePolySoilLayerGMVAsciiBin(Cell(i,j,k)%Cell,i,j,k)
                 END DO
               END DO
             END DO
           END IF
...
SUBROUTINE CountCellOroVolNull(Cell,i,j,k)
  TYPE(Cell_T), POINTER :: Cell
  INTEGER :: i,j,k

  INTEGER :: in_out

  ! Count Ausgabe Orography- Cellen unterhalb Berg mitVolume = Null

  IF (ASSOCIATED(Cell)) THEN
    IF( (Cell%in_out==-6.AND.Cell%vc==0).OR.Cell%in_out==-7 )THEN
       nr_CellOroVolNull=nr_CellOroVolNull+1
    END IF
  ELSE !!! .NOT. ASSOCIATED(Cell) 
     in_out=Vertices(i-1,j-1,k-1)%in_out &
           +Vertices(i,j-1,k-1)%in_out &
           ...
           +Vertices(i,j-1,k)%in_out &
           +Vertices(i,j,k)%in_out
     IF (in_out==-8.OR.in_out==-7) THEN
       nr_CellOroVolNull=nr_CellOroVolNull+1
     END IF
  END IF
END SUBROUTINE CountCellOroVolNull

SUBROUTINE WriteCellOroVolNullGMVAscii(Cell,i,j,k)
  TYPE(Cell_T), POINTER :: Cell
  INTEGER :: i,j,k

  INTEGER :: in_out

   ! Ausgabe Orography- Cellen unterhalb Berg mitVolume = Null
       ! hex
       !----
       !          4--------3    !Point-Folge Output-Hex-GMV-Vorlage-Def
       !         /|       /|
  ...
  IF (ASSOCIATED(Cell)) THEN
    IF( (Cell%in_out==-6.AND.Cell%vc==0).OR.Cell%in_out==-7 )THEN
     !WRITE(10,*)
     !WRITE(10,*) "i,j,k= ",i,j,k,"    ASSOCIATED(Cell), Cell%in_out==-6 und vc=0, in_out=-7"
       WRITE(10,'(a8,I8)') hex,8
       WRITE(10,'(8I8)')  &
          Vertices(i-1,j-1,k)%nrInP &
           ,Vertices(i,j-1,k)%nrInP   &
  ...
  ELSE !!! .NOT. ASSOCIATED(Cell) 
     ...
     END IF
  END IF
END SUBROUTINE WriteCellOroVolNullGMVAscii


SUBROUTINE WriteAllCellsOroVolNullAsciiGMV(FileName)
  CHARACTER*50 :: FileName
  INTEGER :: ib,i,j,k

  OPEN(UNIT=10,FILE=TRIM(FileName)//'.ONull.out.gmvG',STATUS='UNKNOWN')
  CALL Display_OutOroVolNullGMVBlk(FileName)
    ...
    DO ib=1,nb
      ...
      IF(view_cell=='y') THEN
        DO k=v_z0+1,v_z1
          DO j=v_y0+1,v_y1
            DO i=v_x0+1,v_x1
               CALL CountCellOroVolNull(Cell(i,j,k)%Cell,i,j,k)
            END DO
          END DO
        END DO
      END IF
    END DO  !ib
    WRITE(10,'(a8,i8)') cells,nr_CellOroVolNull
    DO ib=1,nb
      CALL Set(Floor(ib))
      CALL Search_OutArea
      IF(view_cell=='y') THEN
        DO k=v_z0+1,v_z1
          DO j=v_y0+1,v_y1
            DO i=v_x0+1,v_x1
               !WRITE(10,*) "---------------------------"
               !WRITE(10,*) "Cell(i,j,k)", i, j, k
               CALL WriteCellOroVolNullGMVAscii(Cell(i,j,k)%Cell,i,j,k)
            END DO
          END DO
        END DO
      END IF
...
END SUBROUTINE WriteAllCellsOroVolNullAsciiGMV
...
SUBROUTINE WriteAllCellsBinGMV(FileName)
  ...
  DO i=1,nr_view_out
    nRec=nRec+1
    tempw=xParametricOut(VertOutView(i))
    WRITE(10,REC=nRec) tempw
  END DO
  !.. WRITE(10,*) VertOutView%Point%y
  DO i=1,nr_view_out
    nRec=nRec+1
    tempw=yParametricOut(VertOutView(i))
    WRITE(10,REC=nRec) tempw
  END DO
  !.. WRITE(10,*) VertOutView%Point%z
  DO i=1,nr_view_out
    nRec=nRec+1
    tempw=zParametricOut(VertOutView(i))
    WRITE(10,REC=nRec) tempw
  END DO
...
SUBROUTINE WriteCellOroVolNullBinGMV(Cell,i,j,k)
  TYPE(Cell_T), POINTER :: Cell
  INTEGER :: i,j,k
  ...
  ELSE !!! .NOT. ASSOCIATED(Cell) 
     ...
     END IF
  END IF
END SUBROUTINE WriteCellOroVolNullBinGMV


SUBROUTINE WriteAllCellsOroVolNullBinGMV(FileName)
  CHARACTER*50 :: FileName
  INTEGER :: ib,i,j,k
  ...
  IF(out_area=='y') THEN
    DO ib=1,nb
      ...
      END IF
    END DO  !ib
    WRITE(10,REC=nRec) nr_CellOroVolNull 
    DO ib=1,nb
      CALL Set(Floor(ib))
      CALL Search_OutArea
      IF(view_cell=='y') THEN
        DO k=v_z0+1,v_z1
          DO j=v_y0+1,v_y1
            DO i=v_x0+1,v_x1
               !WRITE(10,*) "---------------------------"
               !WRITE(10,*) "Cell(i,j,k)", i, j, k
               CALL WriteCellOroVolNullBinGMV(Cell(i,j,k)%Cell,i,j,k)
            END DO
          END DO
        END DO
      END IF
...
END SUBROUTINE WriteAllCellsOroVolNullBinGMV
