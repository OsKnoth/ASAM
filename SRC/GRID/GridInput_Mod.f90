...
!     END IF  !OSSI
 ...
 !     !Block structure
    ...
    ELSE IF (INDEX(Line,'#SelfMultiblock')>0.AND.SelfMultiBlock) THEN
      MultiBlock=.FALSE.
      WRITE(*,*) 'CALL SelfMultiblockCompute'
      CALL SelfMultiblockCompute
      WRITE(*,*) 'Number Blocks',nb
    END IF
    ...
    !-------------------------------------------
       ...
       END IF
       EXIT
    ELSE IF (INDEX(Line,'#LandClassUTM')>0) THEN
      WRITE(*,*) '#LandClassUTM'
    !-------------------------------------------
      READ(InputUnit,*) file_landclass_utm  
      WRITE (*,*) 'Reading ',TRIM(ADJUSTL(file_landclass_utm))
      CALL Read_LandClass_UTM
      nr_landdef=Nr_Land_Cl  !symbolisch
      struct_bound=struct_bound+2 
    END IF
