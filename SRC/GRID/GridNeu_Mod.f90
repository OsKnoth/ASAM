...
SUBROUTINE AnalyzeAllVertices
  ...
  WRITE(*,*) 'Domain%x0, Domain%x1',Domain%x0,Domain%x1
  WRITE(*,*) 'Domain%y0, Domain%y1',Domain%y0,Domain%y1
  WRITE(*,*) 'L_x,dx',Domain%x1-Domain%x0,(Domain%x1-Domain%x0)/Domain%nx
  WRITE(*,*) 'L_y,dy',Domain%y1-Domain%y0,(Domain%y1-Domain%y0)/Domain%ny
  WRITE(*,*) 'L_z,dz',Domain%z1-Domain%z0,(Domain%z1-Domain%z0)/Domain%nz
  WRITE(*,*) 'dxLoc,nx',dxLoc,distx_coeff,Domain%nx
  WRITE(*,*) 'dyLoc,ny',dyLoc,disty_coeff,Domain%ny
  WRITE(*,*) 'dzLoc,nz',dzLoc,distz_coeff,Domain%nz
...
!   dist=MIN(dist,Level(Vertex%Point%x+dxLoc,Vertex%Point%y+dyLoc,Vertex%Point%z))
!   dist=MIN(dist,Level(Vertex%Point%x+dxLoc,Vertex%Point%y-dyLoc,Vertex%Point%z))
!   dist=MIN(dist,Level(Vertex%Point%x-dxLoc,Vertex%Point%y+dyLoc,Vertex%Point%z))
!   dist=MIN(dist,Level(Vertex%Point%x-dxLoc,Vertex%Point%y-dyLoc,Vertex%Point%z))

!   dist=MIN(dist,Level(Vertex%Point%x+dxLoc,Vertex%Point%y,Vertex%Point%z+dzLoc))
!   dist=MIN(dist,Level(Vertex%Point%x+dxLoc,Vertex%Point%y,Vertex%Point%z-dzLoc))
!   dist=MIN(dist,Level(Vertex%Point%x-dxLoc,Vertex%Point%y,Vertex%Point%z+dzLoc))
!   dist=MIN(dist,Level(Vertex%Point%x-dxLoc,Vertex%Point%y,Vertex%Point%z-dzLoc))

!   dist=MIN(dist,Level(Vertex%Point%x,Vertex%Point%y+dyLoc,Vertex%Point%z+dzLoc))
!   dist=MIN(dist,Level(Vertex%Point%x,Vertex%Point%y+dyLoc,Vertex%Point%z-dzLoc))
!   dist=MIN(dist,Level(Vertex%Point%x,Vertex%Point%y-dyLoc,Vertex%Point%z+dzLoc))
!   dist=MIN(dist,Level(Vertex%Point%x,Vertex%Point%y-dyLoc,Vertex%Point%z-dzLoc))

!   dist=MIN(dist,Level(Vertex%Point%x+dxLoc,Vertex%Point%y+dyLoc,Vertex%Point%z+dzLoc))
!   dist=MIN(dist,Level(Vertex%Point%x+dxLoc,Vertex%Point%y+dyLoc,Vertex%Point%z-dzLoc))
!   dist=MIN(dist,Level(Vertex%Point%x+dxLoc,Vertex%Point%y-dyLoc,Vertex%Point%z+dzLoc))
!   dist=MIN(dist,Level(Vertex%Point%x+dxLoc,Vertex%Point%y-dyLoc,Vertex%Point%z-dzLoc))
!   dist=MIN(dist,Level(Vertex%Point%x-dxLoc,Vertex%Point%y+dyLoc,Vertex%Point%z+dzLoc))
...
!   IF (dist>0) THEN
  ...
  ELSE
    ...
    END IF
  END IF
  IF (Vertex%in_out==-1.AND.Vertex%nrp>0) THEN
    STOP 'False Vertex'
  END IF  
...
SUBROUTINE CheckVertexFace(V1,V2,V3,V4,List,ListPos)
  ...
  IF (V1%in_out==-1.AND.V3%in_out==-1.AND. &
    ...
    ListPos=ListPos+1
    WRITE(*,*) 'V1  ',V1%Point,V1%in_out
    WRITE(*,*) 'V2  ',V2%Point,V2%in_out
    WRITE(*,*) 'V3  ',V3%Point,V3%in_out
    WRITE(*,*) 'V4  ',V4%Point,V4%in_out
            ...
            DO jj=MAX(j-1,iy0),MIN(j+is+ks,iy1)
              DO kk=MAX(k-1,iz0),MIN(k+is+js,iz1)
                CALL AllocateEdge(Edges_X(ii,jj,kk)%Edge &
                                 ,Vertices(ii-1,jj,kk)%Vertex,Vertices(ii,jj,kk)%Vertex,'X',ii,jj,kk)
              END DO
            END DO
          END DO
            ...
            DO jj=MAX(j,iy0+1),MIN(j+1-js,iy1)
              DO kk=MAX(k-1,iz0),MIN(k+js+is,iz1)
               CALL AllocateEdge(Edges_Y(ii,jj,kk)%Edge &
                                ,Vertices(ii,jj-1,kk)%Vertex,Vertices(ii,jj,kk)%Vertex,'Y',ii,jj,kk)
              END DO
            END DO
          END DO
            ...
            DO jj=MAX(j-1,iy0),MIN(j+ks+is,iy1)
              DO kk=MAX(k,iz0+1),MIN(k+1-ks,iz1)
                CALL AllocateEdge(Edges_Z(ii,jj,kk)%Edge &
                                 ,Vertices(ii,jj,kk-1)%Vertex,Vertices(ii,jj,kk)%Vertex,'Z',ii,jj,kk)
             END DO
            END DO
          END DO
          DO ii=MAX(i-1,ix0),MIN(i+js+ks,ix1)
...
SUBROUTINE AnalyzeEdge(Edge,Print)
  ...
  IF ((Edge%Vert1%ix<Edge%Vert2%ix.AND..NOT.ASSOCIATED(Edge%Vert1%VertX)).OR. &
    ...
    IF (Edge%Vert1%in_out+Edge%Vert2%in_out==1.OR. &
        Edge%Vert1%in_out*Edge%Vert2%in_out==-1) THEN
      IF (Edge%Vert1%in_out*Edge%Vert2%in_out==-1) THEN
        CALL SearchPoint(VertS,Edge%Vert1,Edge%Vert2)
      ELSE IF (Edge%Vert1%in_out+Edge%Vert2%in_out==-1) THEN  
    ...
    DO in=1,AnzahlNachbar
      ...
      IF (nType=='ie'.OR.nType=='iw') THEN
        ...
        ELSE IF (MIN(RefineNachbarY,RefineNachbarZ)<MIN(RefineY,RefineZ)) THEN
          ! Neighbor coarser
          DO iy=(jy0+IncrY)/IncrY,jy1/IncrY
            DO iz=(jz0+IncrZ)/IncrZ,jz1/IncrZ
              IF (ASSOCIATED(Floor(ibn)%Cells(ix,iy,iz)%Cell)) THEN
                Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol=0.0d0
                DO jy=IncrY*(iy-1)+1,IncrY*iy
                  DO jz=IncrZ*(iz-1)+1,IncrZ*iz
                    IF (ASSOCIATED(CellsNachbar(jx0+1,jy,jz)%Cell)) THEN
        ...
        END IF  
      END IF  
      IF (nType=='in'.OR.nType=='is') THEN
        IF (nType=='in') THEN
          iy=Floor(ibn)%iy0+1
        ELSE
          iy=Floor(ibn)%iy1
        END IF  
        IF (MIN(RefineNachbarX,RefineNachbarZ)>MIN(RefineX,RefineZ)) THEN
          DO jx=jx0+1,jx1
            DO jz=jz0+1,jz1
              IF (ASSOCIATED(CellsNachbar(jx,jy0+1,jz)%Cell)) THEN
                CellsNachbar(jx,jy0+1,jz)%Cell%Vol=0.0d0
                DO ix=IncrX*(jX-1)+1,IncrX*jX
                  DO iz=IncrZ*(jz-1)+1,IncrZ*jz
                    IF (ASSOCIATED(Floor(ibn)%Cells(ix,iy,iz)%Cell)) THEN
                      CellsNachbar(jx,jy0+1,jz)%Cell%Vol=CellsNachbar(jx,jy0+1,jz)%Cell%Vol &
                                                        +Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol
                    ELSE  
                      CellsNachbar(jx,jy0+1,jz)%Cell%Vol=CellsNachbar(jx,jy0+1,jz)%Cell%Vol &
                                                        +Floor(ibn)%dx(ix)*Floor(ibn)%dy(iy)*Floor(ibn)%dz(iz)
            ...
            END DO  
          END DO  
        ELSE IF (MIN(RefineNachbarX,RefineNachbarZ)<MIN(RefineX,RefineZ)) THEN
          DO ix=(jx0+IncrX)/IncrX,jx1/Incrx
            DO iz=(jz0+IncrZ)/IncrZ,jz1/IncrZ
              IF (ASSOCIATED(Floor(ibn)%Cells(ix,iy,iz)%Cell)) THEN
                Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol=0.0d0
                DO jx=IncrX*(ix-1)+1,IncrX*ix
                  DO jz=IncrZ*(iz-1)+1,IncrZ*iz
                    IF (ASSOCIATED(CellsNachbar(jx,jy0+1,jz)%Cell)) THEN
                      Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol=Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol &
                                                         +CellsNachbar(jx,jy0+1,jz)%Cell%Vol
                    ELSE
                      Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol=Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol &
                                                         +dx(jx)*Floor(ibn)%dy(iy)*dz(jz)
                    END IF
                  END DO  
                END DO  
              END IF  
        ...
        END IF  
      END IF  
      IF (nType=='it'.OR.nType=='ib') THEN
        IF (nType=='it') THEN
          iz=Floor(ibn)%iz0+1
        ELSE
          iz=Floor(ibn)%iz1
        END IF  
        IF (MIN(RefineNachbarX,RefineNachbarY)>MIN(RefineX,RefineY)) THEN
          DO jx=jx0+1,jx1
            DO jy=jy0+1,jy1
              IF (ASSOCIATED(CellsNachbar(jx,jy,jz0+1)%Cell)) THEN
                CellsNachbar(jx,jy,jz0+1)%Cell%Vol=0.0d0
                DO ix=IncrX*(jX-1)+1,IncrX*jX
                  DO iy=IncrY*(jy-1)+1,IncrY*jY
                    IF (ASSOCIATED(Floor(ibn)%Cells(ix,iy,iz)%Cell)) THEN
                      CellsNachbar(jx,jy,jz0+1)%Cell%Vol=CellsNachbar(jx,jy,jz0+1)%Cell%Vol &
                                                        +Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol
                    ELSE  
                      CellsNachbar(jx,jy,jz0+1)%Cell%Vol=CellsNachbar(jx,jy,jz0+1)%Cell%Vol &
                                                        +Floor(ibn)%dx(ix)*Floor(ibn)%dy(iy)*Floor(ibn)%dz(iz)
            ...
            END DO  
          END DO  
        ELSE IF (MIN(RefineNachbarX,RefineNachbarY)<MIN(RefineX,RefineY)) THEN
          DO ix=(jx0+IncrX)/IncrX,jx1/IncrX
            DO iy=(jy0+IncrY)/IncrY,jy1/IncrY
              IF (ASSOCIATED(Floor(ibn)%Cells(ix,iy,iz)%Cell)) THEN
                Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol=0.0d0
                DO jx=IncrX*(ix-1)+1,IncrX*ix
                  DO jy=IncrY*(iy-1)+1,IncrY*iy
                    IF (ASSOCIATED(CellsNachbar(jx,jy,jz0+1)%Cell)) THEN
                      Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol=Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol &
                                                         +CellsNachbar(jx,jy,jz0+1)%Cell%Vol
                    ELSE
                      Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol=Floor(ibn)%Cells(ix,iy,iz)%Cell%Vol &
                                                         +dx(jx)*dy(jy)*Floor(ibn)%dz(iz)
                    END IF
                  END DO  
                END DO  
              END IF  
...
SUBROUTINE SetLandClass(Cell,ix,iy)
  ...
  INQUIRE(FILE=file_landclass_utm,EXIST=exist) 
  IF(ASSOCIATED(LandDef)) THEN ! only when #LandClassDef 
    DO nl=1,nr_landdef
      ...
      END IF
    END DO
  ELSE IF (exist) THEN! #LandClassUTM
    CALL SetLandClassUTM(Cell,ix,iy)
  END IF
END SUBROUTINE SetLandClass


SUBROUTINE SetLandClassUTM(Cell,ix,iy)
  TYPE(Cell_T) :: Cell
  INTEGER :: ix,iy

  INTEGER :: igxLoc,igyLoc
  REAL(RealKind) :: XCornerPointL, & ! x left
                    XCornerPointR, & ! x right
                    YCornerPointB, & ! y bottom
                    YCornerPointT    ! y top
  INTEGER :: CountLC(0:9)
  INTEGER :: ArrayMaxLoc(1)
  INTEGER :: jx,jy
  REAL(RealKind) :: dxLoc,dyLoc

  igxLoc=ix*2.e0**RefineX
  igyLoc=iy*2.e0**RefineY

! WRITE(*,*) 'i,p',igxLoc,Domain%xP(ix),igyLoc,Domain%yP(iy)
! Cell%LandClass=v_clc(LandCover(igxLoc,igyLoc))%mat_c
 
! WRITE (*,*) 'SetLandClassUTM:',ix,iy,RefineX,RefineY

! UTM grid = model grid
  IF (Domain%xP(ix)==LandCoverXCoord(ix,iy)*2.e0**RefineX .AND. &
      Domain%xP(ix)==LandCoverXCoord(ix,iy)*2.e0**RefineX .AND. &
      Domain%yP(iy)==LandCoverYCoord(ix,iy)*2.e0**RefineY .AND. &
      Domain%yP(iy)==LandCoverYCoord(ix,iy)*2.e0**RefineY ) THEN
      Cell%LandClass=LandCover(ix,iy)
!     WRITE(*,*) 'SetLandClassUTM: ',ix,iy,LandCover(ix,iy)
  ELSE 
    dxLoc=Domain%xP(igxLoc)-Domain%xP(igxLoc-1)
    dyLoc=Domain%yP(igyLoc)-Domain%yP(igyLoc-1)
    XCornerPointL=Domain%xP(igxLoc)-0.5d0*dxLoc
    XCornerPointR=Domain%xP(igxLoc)+0.5d0*dxLoc
    YCornerPointB=Domain%yP(igyLoc)-0.5d0*dyLoc
    YCornerPointT=Domain%yP(igyLoc)+0.5d0*dyLoc
!   Check if UTM grid cell is within model grid cell
    CountLC=0
    DO jx=1,SIZE(LandCover(:,1))
      DO jy=1,SIZE(LandCover(1,:))
        IF (LandCoverXCoord(jx,jy)*2.e0**RefineX> XCornerPointL .AND. &
            LandCoverXCoord(jx,jy)*2.e0**RefineX<=XCornerPointR .AND. &
            LandCoverYCoord(jx,jy)*2.e0**RefineY> YCornerPointB .AND. &
            LandCoverYCoord(jx,jy)*2.e0**RefineY<=YCornerPointT ) THEN
          SELECT CASE(LandCover(jx,jy))
            CASE(0)       
              CountLC(0)=CountLC(0)+1
            CASE(1)       
              CountLC(1)=CountLC(1)+1
            CASE(2)       
              CountLC(2)=CountLC(2)+1
            CASE(3)       
              CountLC(3)=CountLC(3)+1
            CASE(4)       
              CountLC(4)=CountLC(4)+1
            CASE(5)       
              CountLC(5)=CountLC(5)+1
            CASE(6)       
              CountLC(6)=CountLC(6)+1
            CASE(7)       
              CountLC(7)=CountLC(7)+1
            CASE(8)       
              CountLC(8)=CountLC(8)+1
            CASE(9)       
              CountLC(9)=CountLC(9)+1
            CASE DEFAULT
              WRITE(*,*) 'Error in SUBROUTINE SetLandClassUTM: LandCover(ix,iy) not between 0 and 9.'
              RETURN
          END SELECT
        ELSE IF (Domain%xP(igxLoc)> LandCoverXCoord(jx,jy)-0.5d0*dxUTM .AND. &
                Domain%xP(igxLoc)<=LandCoverXCoord(jx,jy)+0.5d0*dxUTM .AND. &
                Domain%yP(igyLoc)> LandCoverYCoord(jx,jy)-0.5d0*dyUTM .AND. &
                Domain%yP(igyLoc)<=LandCoverYCoord(jx,jy)+0.5d0*dyUTM) THEN
          Cell%LandClass=LandCover(jx,jy)
        END IF  
      END DO ! jy
    END DO ! jx
    IF (SUM(CountLC)>0) THEN ! UTM grid spacing < model grid spacing
      ArrayMaxLoc=MAXLOC(CountLC)-1
      Cell%LandClass=ArrayMaxLoc(1)
    END IF
  END IF

END SUBROUTINE SetLandClassUTM
...
SUBROUTINE AnalyzeCell(Cell,Print)
  ...
  IF (Cell%Face1%NumberVert>2) THEN
    ...
    PosList=PosList+Cell%Face1%NumberVert
    Cell%NumberFace=Cell%NumberFace+1
  END IF
  IF (Cell%Face2%NumberVert>2) THEN
    ...
    PosList=PosList+Cell%Face2%NumberVert
    Cell%NumberFace=Cell%NumberFace+1
  END IF
  IF (Cell%Face3%NumberVert>2) THEN
    ...
    PosList=PosList+Cell%Face3%NumberVert
    Cell%NumberFace=Cell%NumberFace+1
  END IF
  IF (Cell%Face4%NumberVert>2) THEN
    ...
    PosList=PosList+Cell%Face4%NumberVert
    Cell%NumberFace=Cell%NumberFace+1
  END IF
  IF (Cell%Face5%NumberVert>2) THEN
    ...
    PosList=PosList+Cell%Face5%NumberVert
    Cell%NumberFace=Cell%NumberFace+1
  END IF
  IF (Cell%Face6%NumberVert>2) THEN
    ...
    PosList=PosList+Cell%Face6%NumberVert
    Cell%NumberFace=Cell%NumberFace+1
  END IF

  ALLOCATE(VertexList(PosList))
  PosList=1
  IF (Cell%Face1%NumberVert>2) THEN
    VertexList(PosList:PosList+Cell%Face1%NumberVert-1)=Cell%Face1%VertexList(1:Cell%Face1%NumberVert)
    PosList=PosList+Cell%Face1%NumberVert
  END IF
  IF (Cell%Face2%NumberVert>2) THEN
    VertexList(PosList:PosList+Cell%Face2%NumberVert-1)=Cell%Face2%VertexList(1:Cell%Face2%NumberVert)
    PosList=PosList+Cell%Face2%NumberVert
  END IF
  IF (Cell%Face3%NumberVert>2) THEN
    VertexList(PosList:PosList+Cell%Face3%NumberVert-1)=Cell%Face3%VertexList(1:Cell%Face3%NumberVert)
    PosList=PosList+Cell%Face3%NumberVert
  END IF
  IF (Cell%Face4%NumberVert>2) THEN
    VertexList(PosList:PosList+Cell%Face4%NumberVert-1)=Cell%Face4%VertexList(1:Cell%Face4%NumberVert)
    PosList=PosList+Cell%Face4%NumberVert
  END IF
  IF (Cell%Face5%NumberVert>2) THEN
    VertexList(PosList:PosList+Cell%Face5%NumberVert-1)=Cell%Face5%VertexList(1:Cell%Face5%NumberVert)
    PosList=PosList+Cell%Face5%NumberVert
  END IF
  IF (Cell%Face6%NumberVert>2) THEN
    VertexList(PosList:PosList+Cell%Face6%NumberVert-1)=Cell%Face6%VertexList(1:Cell%Face6%NumberVert)
    PosList=PosList+Cell%Face6%NumberVert
  END IF
  CALL CompressList(VertexList)
  Cell%NumberVert=SIZE(VertexList)
  Cell%VertList=>VertexList
  IF (PRESENT(Print)) WRITE(*,*) 'vor Traverse VertexList'
  C1:DO i=1,6
    C2:DO iVert=1,8
  ...
  END DO C1
  ListCut1=ListCut
  IF (PRESENT(Print)) WRITE(*,*) 'nCut ',nCut
  IF (nCut>0) THEN
    Cell%NumberFace=Cell%NumberFace+1
    nCutNeu=nCut
    DO i=1,ncut-1
    ...
    ListCut(1,1)=0
    ListCut(2,1)=0
    IF (PRESENT(Print)) WRITE(*,*) 'nCutNeu ',nCutNeu
    iv=2
    S2:DO
      S1:DO i=1,nCut
        ...
        END IF
      END DO S1
      IF (PRESENT(Print)) WRITE(*,*) 'nach S1 ',i,iv,nCut,nCutNeu
      IF (Cell%VertCut(1)==Cell%VertCut(iv).AND.iv<nCutNeu) THEN
        Cell%Unresolved=.TRUE.
        WRITE(*,*) 'More than one Loop',Cell%Unresolved,Cell%ix,Cell%iy,Cell%iz
        DO i=1,nCut
          WRITE(*,*) i,'ListCut',ListCut1(:,i)
        END DO  
  ...
  WRITE(*,*) 'EdgP0',Cell%Face1%Edge1%Vert1%Point,Cell%Face1%Edge1%Vert1%nrp,Cell%Face1%Edge1%Vert1%in_out
  WRITE(*,*) 'EdgP1',Cell%Face1%Edge1%Vert2%Point,Cell%Face1%Edge1%Vert2%nrp,Cell%Face1%Edge1%Vert2%in_out
  WRITE(*,*) 'EdgP2',Cell%Face1%Edge3%Vert1%Point,Cell%Face1%Edge3%Vert1%nrp,Cell%Face1%Edge3%Vert1%in_out
  WRITE(*,*) 'EdgP3',Cell%Face1%Edge3%Vert2%Point,Cell%Face1%Edge3%Vert2%nrp,Cell%Face1%Edge3%Vert2%in_out
  WRITE(*,*) 'EdgP4',Cell%Face2%Edge1%Vert1%Point,Cell%Face2%Edge1%Vert1%nrp,Cell%Face2%Edge1%Vert1%in_out
  WRITE(*,*) 'EdgP5',Cell%Face2%Edge1%Vert2%Point,Cell%Face2%Edge1%Vert2%nrp,Cell%Face2%Edge1%Vert2%in_out
  WRITE(*,*) 'EdgP6',Cell%Face2%Edge3%Vert1%Point,Cell%Face2%Edge3%Vert1%nrp,Cell%Face2%Edge3%Vert1%in_out
  WRITE(*,*) 'EdgP7',Cell%Face2%Edge3%Vert2%Point,Cell%Face2%Edge3%Vert2%nrp,Cell%Face2%Edge3%Vert2%in_out
  CALL WriteVertDebug(Cell%Face1%Edge1%Vert1)
  CALL WriteVertDebug(Cell%Face1%Edge1%Vert2)
  CALL WriteVertDebug(Cell%Face1%Edge3%Vert1)
  CALL WriteVertDebug(Cell%Face1%Edge3%Vert2)
  CALL WriteVertDebug(Cell%Face2%Edge1%Vert1)
  CALL WriteVertDebug(Cell%Face2%Edge1%Vert2)
  CALL WriteVertDebug(Cell%Face2%Edge3%Vert1)
  CALL WriteVertDebug(Cell%Face2%Edge3%Vert2)
!       STOP 'More than one Loop'
        WRITE(*,*) 'More than one Loop',Cell%vc
        Cell%vc=iv 
        EXIT S2
      END IF  
      IF (iv>=nCutNeu) THEN
  ...
  ELSE
    ...
    END IF
  END IF
  IF (PRESENT(Print)) WRITE(*,*) 'vor Volumenberechnung'
  ...
  IF (Cell%Vol<=RelVol*Cell%VolFull) THEN
...
SUBROUTINE AllocateEdge(Edge,Vert1,Vert2,EdgeType,ix,iy,iz)
...
SUBROUTINE AllocateFace(Face,Edge1,Edge2,Edge3,Edge4,Vol,FaceType,ix,iy,iz)
...
SUBROUTINE AllocateCell(Cell,Face1,Face2,Face3,Face4,Face5,Face6,Vol,ix,iy,iz)
  ...
  END IF
  Cell%Vol=Vol(Cell)
  Cell%VolFull=Cell%Vol
  Cell%ix=ix
  Cell%iy=iy
  Cell%iz=iz
END SUBROUTINE AllocateCell

SUBROUTINE SearchPoint(InterVert,Vertex1,Vertex2,Debug)
  TYPE (Vertex_T) :: InterVert
  TYPE (Vertex_T) :: Vertex1,Vertex2
  ...
  IF (PRESENT(Debug)) WRITE(*,*) 'in SearchPoint'
  xL=Vertex1%Point%x
  yL=Vertex1%Point%y
  zL=Vertex1%Point%z
  xR=Vertex2%Point%x
