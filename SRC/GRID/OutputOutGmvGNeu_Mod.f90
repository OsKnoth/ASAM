...
SUBROUTINE WriteCellNodes(Cell,i,j,k)
  TYPE(Cell_T), POINTER :: Cell
  INTEGER :: i,j,k
  INTEGER :: iVert
  INTEGER :: in_out
  INTEGER :: VertList(20),iList
    ...
    IF ((Cell%in_out>-8.AND.Cell%Vol>0.0d0) &
      ...
      IF (Cell%vc==0) THEN
       ! hex
        ...
        DO iVert=1,4
          iList=iList+1
          VertList(iList)=VertViewScale(Cell%Face2%VertexList(iVert))-1
        END DO
        DO iVert=1,4
          iList=iList+1
          VertList(iList)=VertViewScale(Cell%Face1%VertexList(iVert))-1
        END DO
        WRITE(10,*) VertList(1:iList)-1
      ELSE  
        iList=0
        DO iVert=1,Cell%NumberVert
          iList=iList+1
          VertList(iList)=VertViewScale(Cell%VertList(iVert))
        END DO  
        WRITE(10,*) VertList(1:iList)-1
      END IF
    END IF
  ELSE
    ...
    IF (in_out>=0) THEN
     ! hex
      ...
      !        |/       |/
      !        5--------6
      VertList(1)=VertViewScale(Vertices(i-1,j-1,k)%Vertex%nrP)
      VertList(2)=VertViewScale(Vertices(i,j-1,k)%Vertex%nrP)
      VertList(3)=VertViewScale(Vertices(i,j,k)%Vertex%nrP)
      VertList(4)=VertViewScale(Vertices(i-1,j,k)%Vertex%nrP)
      VertList(5)=VertViewScale(Vertices(i-1,j-1,k-1)%Vertex%nrP)
      VertList(6)=VertViewScale(Vertices(i,j-1,k-1)%Vertex%nrP)
      VertList(7)=VertViewScale(Vertices(i,j,k-1)%Vertex%nrP)
      VertList(8)=VertViewScale(Vertices(i-1,j,k-1)%Vertex%nrP)
      WRITE(10,*) VertList(1:8)-1
    END IF
  END IF

END SUBROUTINE WriteCellNodes

SUBROUTINE WriteCellFaces(Cell,i,j,k)
  TYPE(Cell_T), POINTER :: Cell
  INTEGER :: i,j,k
  INTEGER :: iVert
  INTEGER :: in_out
    ...
    IF ((Cell%in_out>-8.AND.Cell%Vol>0.0d0) &
      ...
      IF (Cell%vc==0) THEN
       ! hex
        ...
        END DO
        WRITE(10,*) 6
        WRITE(10,*) 4,VertList(1),VertList(2),VertList(3),VertList(4)
        WRITE(10,*) 4,VertList(5),VertList(8),VertList(7),VertList(6)
        WRITE(10,*) 4,VertList(6),VertList(7),VertList(3),VertList(2)
        WRITE(10,*) 4,VertList(5),VertList(1),VertList(4),VertList(8)
        WRITE(10,*) 4,VertList(5),VertList(6),VertList(2),VertList(1)
        WRITE(10,*) 4,VertList(8),VertList(4),VertList(3),VertList(7)
      ELSE  
        WRITE(10,*) Cell%NumberFace
        ...
        DO iVert=1,Cell%vc
          iList=iList+1
          VertList(iList)=VertViewScale(Cell%VertCut(iVert))
        END DO
        WRITE(10,*) iList,VertList(1:iList)-1  
      END IF
    END IF
  ELSE
    in_out=Vertices(i-1,j-1,k-1)%Vertex%in_out &
    ...
    IF (in_out>=0) THEN
     ! hex
      ...
      !        |/       |/
      !        5--------6
      VertList(1)=VertViewScale(Vertices(i-1,j-1,k)%Vertex%nrP)-1
      VertList(2)=VertViewScale(Vertices(i,j-1,k)%Vertex%nrP)-1
      VertList(3)=VertViewScale(Vertices(i,j,k)%Vertex%nrP)-1
      VertList(4)=VertViewScale(Vertices(i-1,j,k)%Vertex%nrP)-1
      VertList(5)=VertViewScale(Vertices(i-1,j-1,k-1)%Vertex%nrP)-1
      VertList(6)=VertViewScale(Vertices(i,j-1,k-1)%Vertex%nrP)-1
      VertList(7)=VertViewScale(Vertices(i,j,k-1)%Vertex%nrP)-1
      VertList(8)=VertViewScale(Vertices(i-1,j,k-1)%Vertex%nrP)-1
      WRITE(10,*) 6
      WRITE(10,*) 4,VertList(1),VertList(2),VertList(3),VertList(4)
      WRITE(10,*) 4,VertList(5),VertList(8),VertList(7),VertList(6)
      WRITE(10,*) 4,VertList(6),VertList(7),VertList(3),VertList(2)
...
SUBROUTINE WriteFace(Face)
  ...
  IF (Face%NumberVert>2) THEN
     iList=0
     DO iVert=1,Face%NumberVert
       iList=iList+1
       VertList(iList)=VertViewScale(Face%VertexList(iVert))
     END DO
     WRITE(10,*) iList,VertList(1:iList)-1  
  END IF
END SUBROUTINE WriteFace

END SUBROUTINE WriteCellFaces

SUBROUTINE WriteCellFacesOffset(Cell,i,j,k,Offset)
  TYPE(Cell_T), POINTER :: Cell
  INTEGER :: i,j,k
  INTEGER :: Offset
  INTEGER :: iVert
  ...
  IF (ASSOCIATED(Cell)) THEN
    IF ((Cell%in_out>-8.AND.Cell%Vol>0.0d0) &
      ...
      IF (Cell%vc==0) THEN
       ! hex
        Offset=Offset+1+6+4*6
        WRITE(10,*) Offset
      ELSE  
        Offset=Offset+1
        CALL FaceOffset(Cell%Face1,Offset)
        CALL FaceOffset(Cell%Face2,Offset)
        CALL FaceOffset(Cell%Face3,Offset)
        CALL FaceOffset(Cell%Face4,Offset)
        CALL FaceOffset(Cell%Face5,Offset)
        CALL FaceOffset(Cell%Face6,Offset)
        Offset=Offset+1+Cell%vc
        WRITE(10,*) Offset
      END IF
    END IF
  ELSE
  ...
  END IF
CONTAINS  
SUBROUTINE FaceOffset(Face,Offset)
  TYPE(Face_T), POINTER :: Face
  INTEGER :: Offset

  IF (Face%NumberVert>2) THEN
    Offset=Offset+1+Face%NumberVert 
  END IF
END SUBROUTINE FaceOffset
END SUBROUTINE WriteCellFacesOffset

SUBROUTINE WriteCellNodesOffset(Cell,i,j,k,Offset)
  TYPE(Cell_T), POINTER :: Cell
  INTEGER :: i,j,k
  INTEGER :: Offset
...
END SUBROUTINE WriteCellNodesOffSet

SUBROUTINE WriteAllCellsVTK(FileName)
  CHARACTER(*) :: FileName
  ...
  OPEN(UNIT=10,FILE=TRIM(FileName)//'.vtu',STATUS='UNKNOWN')
  WRITE(10,'(a21)') '<?xml version="1.0"?>'
  WRITE(10,'(a33)') '<VTKFile type="UnstructuredGrid">'
  WRITE(10,'(a18)') '<UnstructuredGrid>'
  WRITE(10,'(a23,i8,a17,i8,a3)') '<Piece NumberOfPoints="',nr_view_out,'" NumberOfCells="',NumberCellsGMV,'">'

  WRITE(10,'(a8)') '<Points>'
  WRITE(10,'(a64)') '<DataArray type="Float32" NumberOfComponents="3" format="ascii">'
  DO iNode=1,nr_view_out
    temp=xParametricOut(VertOutView(iNode)%Vertex)
    WRITE(10,*) temp
    temp=yParametricOut(VertOutView(iNode)%Vertex)
    WRITE(10,*) temp
    temp=zParametricOut(VertOutView(iNode)%Vertex)
    WRITE(10,*) temp
  END DO  
  ...
  DO ib=1,nb
    ...
    IF(view_cell=='y') THEN
      DO iz=v_z0+1,v_z1
        DO iy=v_y0+1,v_y1
          DO ix=v_x0+1,v_x1
            CALL WriteCellNodes(Cells(ix,iy,iz)%Cell,ix,iy,iz)
          END DO
        END DO
      END DO
    END IF  
  ...
  DO ib=1,nb
    ...
    IF(view_cell=='y') THEN
      DO iz=v_z0+1,v_z1
        DO iy=v_y0+1,v_y1
          DO ix=v_x0+1,v_x1
            CALL WriteCellNodesOffset(Cells(ix,iy,iz)%Cell,ix,iy,iz,iOffsets)
          END DO
        END DO
      END DO
    END IF  
  ...
  DO ib=1,nb
    ...
    IF(view_cell=='y') THEN
      DO iz=v_z0+1,v_z1
        DO iy=v_y0+1,v_y1
          DO ix=v_x0+1,v_x1
            CALL WriteCellFaces(Cells(ix,iy,iz)%Cell,ix,iy,iz)
          END DO
        END DO
      END DO
    END IF  
  ...
  DO ib=1,nb
    ...
    IF(view_cell=='y') THEN
      DO iz=v_z0+1,v_z1
        DO iy=v_y0+1,v_y1
          DO ix=v_x0+1,v_x1
            CALL WriteCellFacesOffset(Cells(ix,iy,iz)%Cell,ix,iy,iz,iFaceOffsets)
          END DO
        END DO
      END DO
    END IF  
...
END SUBROUTINE WriteAllCellsVTK

SUBROUTINE WriteAllBlocksVTK(FileName)
  CHARACTER(*) :: FileName
  ...
  OPEN(UNIT=10,FILE=TRIM(FileName)//'_Block.vtu',STATUS='UNKNOWN')
  WRITE(10,'(a21)') '<?xml version="1.0"?>'
  WRITE(10,'(a33)') '<VTKFile type="UnstructuredGrid">'
  WRITE(10,'(a18)') '<UnstructuredGrid>'
  WRITE(10,'(a23,i8,a17,i8,a3)') '<Piece NumberOfPoints="',8*nb,'" NumberOfCells="',nb,'">'
  WRITE(10,'(a46)') '<CellData Scalars="scalars" Vector="Velocity">'
  WRITE(10,'(a56)') '<DataArray type="Float64" Name="Density" Format="ascii">'
  DO ib=1,nb
    WRITE(10,*) Floor(ib)%nc
  END DO  
  WRITE(10,'(a12)') '</DataArray>'
  ...
  DO ib=1,nb
    CALL Set(Floor(ib))
    temp=xParametricOut(Vertices(ix0,iy0,iz1)%Vertex)
    WRITE(10,*) temp
    temp=yParametricOut(Vertices(ix0,iy0,iz1)%Vertex)
    WRITE(10,*) temp
    temp=zParametricOut(Vertices(ix0,iy0,iz1)%Vertex)
    WRITE(10,*) temp

    temp=xParametricOut(Vertices(ix1,iy0,iz1)%Vertex)
    WRITE(10,*) temp
    temp=yParametricOut(Vertices(ix1,iy0,iz1)%Vertex)
    WRITE(10,*) temp
    temp=zParametricOut(Vertices(ix1,iy0,iz1)%Vertex)
    WRITE(10,*) temp

    temp=xParametricOut(Vertices(ix1,iy1,iz1)%Vertex)
    WRITE(10,*) temp
    temp=yParametricOut(Vertices(ix1,iy1,iz1)%Vertex)
    WRITE(10,*) temp
    temp=zParametricOut(Vertices(ix1,iy1,iz1)%Vertex)
    WRITE(10,*) temp

    temp=xParametricOut(Vertices(ix0,iy1,iz1)%Vertex)
    WRITE(10,*) temp
    temp=yParametricOut(Vertices(ix0,iy1,iz1)%Vertex)
    WRITE(10,*) temp
    temp=zParametricOut(Vertices(ix0,iy1,iz1)%Vertex)
    WRITE(10,*) temp

    temp=xParametricOut(Vertices(ix0,iy0,iz0)%Vertex)
    WRITE(10,*) temp
    temp=yParametricOut(Vertices(ix0,iy0,iz0)%Vertex)
    WRITE(10,*) temp
    temp=zParametricOut(Vertices(ix0,iy0,iz0)%Vertex)
    WRITE(10,*) temp

    temp=xParametricOut(Vertices(ix1,iy0,iz0)%Vertex)
    WRITE(10,*) temp
    temp=yParametricOut(Vertices(ix1,iy0,iz0)%Vertex)
    WRITE(10,*) temp
    temp=zParametricOut(Vertices(ix1,iy0,iz0)%Vertex)
    WRITE(10,*) temp

    temp=xParametricOut(Vertices(ix1,iy1,iz0)%Vertex)
    WRITE(10,*) temp
    temp=yParametricOut(Vertices(ix1,iy1,iz0)%Vertex)
    WRITE(10,*) temp
    temp=zParametricOut(Vertices(ix1,iy1,iz0)%Vertex)
    WRITE(10,*) temp

    temp=xParametricOut(Vertices(ix0,iy1,iz0)%Vertex)
    WRITE(10,*) temp
    temp=yParametricOut(Vertices(ix0,iy1,iz0)%Vertex)
    WRITE(10,*) temp
    temp=zParametricOut(Vertices(ix0,iy1,iz0)%Vertex)
    WRITE(10,*) temp
  END DO  
  WRITE(10,'(a12)') '</DataArray>'
  WRITE(10,'(a9)') '</Points>'
  ...
  iNode=0
  DO ib=1,nb
    WRITE(10,*) iNode+0,iNode+1,iNode+2,iNode+3,iNode+4,iNode+5,iNode+6,iNode+7
    iNode=iNode+8
  END DO 
  WRITE(10,'(a12)') '</DataArray>'
  ...
  DO ib=1,nb
    WRITE(10,*) 6
    WRITE(10,*) 4,iNode+1,iNode+2,iNode+3,iNode+4
    WRITE(10,*) 4,iNode+5,iNode+8,iNode+7,iNode+6
    WRITE(10,*) 4,iNode+6,iNode+7,iNode+3,iNode+2
    WRITE(10,*) 4,iNode+5,iNode+1,iNode+4,iNode+8
    WRITE(10,*) 4,iNode+5,iNode+6,iNode+2,iNode+1
    WRITE(10,*) 4,iNode+8,iNode+4,iNode+3,iNode+7
    iNode=iNode+8
  END DO 
  WRITE(10,'(a12)') '</DataArray>'
  WRITE(10,'(a58)') '<DataArray type="Int32" Name="faceoffsets" format="ascii">'
  iFaceOffsets=0
  DO ib=1,nb
    iFaceOffsets=iFaceOffsets+1+6+4*6
...
END SUBROUTINE WriteAllBlocksVTK

SUBROUTINE WriteUnresolvedCellsVTK(FileName)
  CHARACTER(*) :: FileName
  
  INTEGER :: NumberCells,NumberPoints
  INTEGER :: ib,ix,iy,iz
  INTEGER :: iOffset=0
  REAL(8) :: temp
  REAL(8), ALLOCATABLE :: Unresolved(:,:)
  ...
  DO ib=1,nb
    CALL Set(Floor(ib))
    IF (iz0==0) THEN
      NumberCells=(iy1-iy0)*(ix1-ix0)+NumberCells
      NumberPoints=(iy1-iy0+1)*(ix1-ix0+1)+NumberPoints
    END IF  
  END DO  


  OPEN(UNIT=10,FILE=TRIM(FileName)//'_UnresC'//'.vtu',STATUS='UNKNOWN')
  WRITE(10,'(a21)') '<?xml version="1.0"?>'
  WRITE(10,'(a33)') '<VTKFile type="UnstructuredGrid">'
  WRITE(10,'(a18)') '<UnstructuredGrid>'
  WRITE(10,'(a23,i8,a17,i8,a3)') '<Piece NumberOfPoints="',NumberPoints,'" NumberOfCells="',NumberCells,'">'
  WRITE(10,'(a28)') '<CellData Scalars="scalars">'
  WRITE(10,*) '<DataArray type="Float64" Name="'//'ColorFace'//'" Format="ascii">'
  DO ib=1,nb
    CALL Set(Floor(ib))
    IF (iz0==0) THEN
      ALLOCATE(Unresolved(ix0+1:ix1,iy0+1:iy1))
      Unresolved=0.0d0
      DO iy=iy0+1,iy1
        DO ix=ix0+1,ix1
          DO iz=iz0+1,iz1
            IF (ASSOCIATED(Cells(ix,iy,iz)%Cell)) THEN 
              IF (Cells(ix,iy,iz)%Cell%Unresolved) THEN
!               WRITE(10,*) 1
                ixMin=MAX(ix0+1,ix-4)
                ixMax=MIN(ix1,ix+4)
                iyMin=MAX(iy0+1,iy-4)
                iyMax=MIN(iy1,iy+4)
                WRITE(*,*) 'Unresolved Cell',ix,iy
                Unresolved(ixMin:ixMax,iyMin:iyMax)=Unresolved(ixMin:ixMax,iyMin:iyMax)+1.0d0
                EXIT
              END IF  
            ELSE  
!             WRITE(10,*) 0
  ...
  DO ib=1,nb
    ...
    IF (iz0==0) THEN
      DO iy=iy0,iy1
        DO ix=ix0,ix1
          temp=xParametricOut(Vertices(ix,iy,iz0)%Vertex)
          WRITE(10,*) temp
          temp=yParametricOut(Vertices(ix,iy,iz0)%Vertex)
          WRITE(10,*) temp
          temp=zParametricOut(Vertices(ix,iy,iz0)%Vertex)
          WRITE(10,*) .01d0
        END DO  
      END DO  
    END IF
  ...
  DO ib=1,nb
    ...
    IF (iz0==0) THEN
      DO iy=iy0+1,iy1
        DO ix=ix0+1,ix1
!         WRITE(10,*) ix-ix0-1+(iy-iy0-1)*(ix1-ix0+1)+iOffset &
!                    ,ix-ix0+(iy-iy0-1)*(ix1-ix0+1)+iOffset &
!                    ,ix-ix0+(iy-iy0)*(ix1-ix0+1)+iOffset &
!                    ,ix-ix0-1+(iy-iy0)*(ix1-ix0+1)+iOffset 
          WRITE(10,*) ix-ix0-1+(iy-iy0-1)*(ix1-ix0+1)+iOffset &
                     ,ix-ix0-1+(iy-iy0)*(ix1-ix0+1)+iOffset &
                     ,ix-ix0+(iy-iy0)*(ix1-ix0+1)+iOffset &
                     ,ix-ix0+(iy-iy0-1)*(ix1-ix0+1)+iOffset 
        END DO  
      END DO  
      iOffset=iOffset+(ix1-ix0+1)*(iy1-iy0+1)
    END IF  
  END DO 
  WRITE(10,'(a12)') '</DataArray>'
...
END SUBROUTINE WriteUnresolvedCellsVTK
...
SUBROUTINE WriteAllCellsCutPlaneBinGMV(FileName)
  ...
  ELSE
    ...
    END DO  !ib
  END IF
  WRITE(*,*) 'End1 WriteAllCellsCutPlaneBinGMV'
  ...
  CLOSE(10)
  WRITE(*,*) 'End2 WriteAllCellsCutPlaneBinGMV'
