...
SUBROUTINE WriteUnstructuredGrid(FileName)
        ...
        DO ix=ix0+1,ix1
          IF (Edges_X(ix,iy,iz)%Number>0) THEN
            WRITE(OutputUnit,*) 'ex  ',Edges_X(ix,iy,iz)%Number,Vertices(ix-1,iy,iz)%Number &
                                      ,Vertices(ix,iy,iz)%Number,ix-1,iy,iz,'c1'
        ...
        DO ix=ix0,ix1
          IF (Edges_Y(ix,iy,iz)%Number>0) THEN
            WRITE(OutputUnit,*) 'ey  ',Edges_Y(ix,iy,iz)%Number,Vertices(ix,iy-1,iz)%Number &
                                      ,Vertices(ix,iy,iz)%Number,ix,iy-1,iz,'c1'
          ...
          ELSE
            ...
            ELSE
              ...
              WRITE(OutputUnit,*) 'eyc ',Edges_Y(ix,iy,iz)%Edge%Number+1,Vertices(ix,iy-1,iz)%Number &
                                        ,Edges_Y(ix,iy,iz)%Edge%NumberNode,ix,iy-1,iz,'c6'
            END IF
          END IF  
        END DO  
      END DO  
        ...
        DO ix=ix0,ix1
          IF (Edges_Z(ix,iy,iz)%Number>0) THEN
            WRITE(OutputUnit,*) 'ez  ',Edges_Z(ix,iy,iz)%Number,Vertices(ix,iy,iz-1)%Number &
                                      ,Vertices(ix,iy,iz)%Number,ix,iy,iz-1,'c1'
          ...
          ELSE
            ...
            ELSE
              ...
              WRITE(OutputUnit,*) 'ezc ',Edges_Z(ix,iy,iz)%Edge%Number+1,Vertices(ix,iy,iz-1)%Number &
                                        ,Edges_Z(ix,iy,iz)%Edge%NumberNode,ix,iy,iz-1,'c6'
            END IF
          END IF  
        END DO  
      END DO  
  ...
  DO ib=1,nb
    ...
    DO iz=iz0,iz1
      DO iy=iy0+1,iy1
        DO ix=ix0+1,ix1
          CALL WriteEdgeCut(Faces_XY(ix,iy,iz),OutputUnit,ix,iy,iz)
        END DO  
      END DO  
    END DO  
    DO iz=iz0+1,iz1
      DO iy=iy0+1,iy1
        DO ix=ix0,ix1
          CALL WriteEdgeCut(Faces_YZ(ix,iy,iz),OutputUnit,ix,iy,iz)
        END DO  
      END DO  
    END DO  
    DO iz=iz0+1,iz1
      DO iy=iy0,iy1
        DO ix=ix0+1,ix1
          CALL WriteEdgeCut(Faces_ZX(ix,iy,iz),OutputUnit,ix,iy,iz)
        END DO  
      END DO  
    END DO  
  END DO  
    ...
    DO iz=iz0,iz1
      DO iy=iy0+1,iy1
        DO ix=ix0+1,ix1
          ...
          ELSE
            CALL WriteCutFace(Faces_XY(ix,iy,iz),OutputUnit,'xy',ix,iy,iz)
          END IF                            
        END DO  
      END DO  
    END DO  
    DO iz=iz0+1,iz1
      DO iy=iy0+1,iy1
        DO ix=ix0,ix1
          ...
          ELSE
            CALL WriteCutFace(Faces_YZ(ix,iy,iz),OutputUnit,'yz',ix,iy,iz)
          END IF                            
        END DO  
      END DO  
    END DO  
    DO iz=iz0+1,iz1
      DO iy=iy0,iy1
        DO ix=ix0+1,ix1
          ...
          ELSE
            CALL WriteCutFace(Faces_ZX(ix,iy,iz),OutputUnit,'zx',ix,iy,iz)
          END IF                            
        END DO  
      END DO  
    END DO  
  ...
  DO ib=1,nb
    ...
    DO iz=iz0+1,iz1
      DO iy=iy0+1,iy1
        DO ix=ix0+1,ix1
          CALL WriteFaceCut(Cells(ix,iy,iz),OutputUnit,ix,iy,iz)
        END DO  
      END DO  
    END DO  
  END DO  
...
SUBROUTINE WriteCutFace(FaceP,OutputUnit,Type,ix,iy,iz)
  TYPE(FaceP_T) :: FaceP
  INTEGER :: OutputUnit
  CHARACTER*2 :: Type
  INTEGER :: ix,iy,iz
  ...
  IF (ASSOCIATED(Face).AND.Face%NumberEdge>0) THEN
    ...
    ELSE  
      ...
      END IF  
    END IF  
    WRITE(OutputUnit,*) 'f'//Type//'c',Face%Number,iList1,1,ix,iy,iz,'f1'
    WRITE(OutputUnit,*) List1Edge(1:iList1)
    WRITE(OutputUnit,*) 'f'//Type//'c',Face%Number+1,iList2,1,ix,iy,iz,'f2'
...
SUBROUTINE WriteFaceCut(CellP,OutputUnit,ix,iy,iz)
  TYPE(CellP_T) :: CellP
  INTEGER :: OutputUnit
  INTEGER :: ix,iy,iz
  ...
  IF (ASSOCIATED(Cell).AND.Cell%NumberFace>0) THEN
    ...
    END IF  
    IF (iList>=3) THEN
      WRITE(OutputUnit,*) 'fc  ',Cell%NumberFace,iList,-1,ix,iy,iz,'cf'
...
SUBROUTINE WriteEdgeCut(FaceP,OutputUnit,ix,iy,iz)
  TYPE(FaceP_T) :: FaceP
  INTEGER :: OutputUnit
  INTEGER :: ix,iy,iz
  TYPE(Face_T), POINTER :: Face
...
SUBROUTINE CountEdge(EdgeP,NumberEdges,NumberNodes)
  ...
  IF (ASSOCIATED(Edge)) THEN
    SELECT CASE (Edge%EdgeType)
      CASE ('X')
        IF (ASSOCIATED(Edge%Vert1%VertX).AND.Edge%Vert1%in_out*Edge%Vert2%in_out==-1) THEN
        ...
        END IF  
      CASE ('Y')
        IF (ASSOCIATED(Edge%Vert1%VertY).AND.Edge%Vert1%in_out*Edge%Vert2%in_out==-1) THEN
          NumberEdges=NumberEdges+1
          Edge%Number=NumberEdges
          NumberNodes=NumberNodes+1
          Edge%NumberNode=NumberNodes
        ...
        END IF  
      CASE ('Z')
        IF (ASSOCIATED(Edge%Vert1%VertZ).AND.Edge%Vert1%in_out*Edge%Vert2%in_out==-1) THEN
          NumberEdges=NumberEdges+1
          Edge%Number=NumberEdges
          NumberNodes=NumberNodes+1
          Edge%NumberNode=NumberNodes
