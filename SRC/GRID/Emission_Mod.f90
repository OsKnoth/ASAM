...
SUBROUTINE InputEmission(Filename)
  CHARACTER(*) :: FileName
  ...
  DO
    ...
    IF (INDEX(Line,'#Emission')>0) THEN
      READ(InputUnit,*) file_nameemi
      file_nameemi=ADJUSTL(file_nameemi)
    END IF  
  END DO  
1 CONTINUE  
END SUBROUTINE InputEmission
