...
SUBROUTINE DataCompute
  ...
  VegFactor    = MAX(0.0d0,MIN(1.0d0,1.12d0*SIN(Pi*MAX(0.0d0,(REAL(StartDay)-VegBegin)/VegPeriod))))
  HeightReduct = 1.0d0 ! EXP(-5.0d-9*GeopotentialHeightOfSurface**2.0d0)
  !RITE (*,*) 'DataCompute',latitude,VegBegin,VegPeriod,REAL(StartDay),VegFactor,1.12d0*SIN(Pi*(REAL(StartDay)-VegBegin)/VegPeriod)
...
FUNCTION PlantCover(LandUse)
  ...
  INTEGER        :: LandUse
  CALL DataCompute
  !RITE(*,*) 'Function PlantCover',PlantCover_Min(LandUse),PlantCover_Max(LandUse),VegFactor,HeightReduct
