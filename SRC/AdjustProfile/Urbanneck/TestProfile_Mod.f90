...
SUBROUTINE ResMoist(z,y,yprime,delta,ires,rpar,ipar)
  ...
  REAL(RealKind) :: Tu,To,zu,zo
  REAL(RealKind) :: RelHu,RelHo,QVu,QVo
  REAL(RealKind) :: p,pPrime,Te,Theta,Rho,RhoD,RhoV,RelH,QV
...
! \rho   y(4)
  ...
  Delta(4)=Te-((z-zu)*To+(zo-z)*Tu)/(zo-zu)
  Delta(5)=RhoV-RelH*SatVap(Te)/(Rv*Te) ! RhoV
  Delta(6)=RelH-QV*Rv*Te*Rho/SatVap(Te)
  Delta(7)=QV-((z-zu)*QVo+(zo-z)*QVu)/(zo-zu)
  WRITE(*,*) 'Delta ',Delta(1:7)

END SUBROUTINE ResMoistQV

SUBROUTINE InputRes(y,yprime,rpar,ipar,prestart,rhstart,teStart,qvstart)
  ...
  SELECT CASE(Case)
    ...
    CASE(Moist,MoistQV)
      ...
      ELSE
        ...
        rpar(8)=1.0d-3
      END IF  
      IF (PRESENT(teStart).AND.PRESENT(preStart).AND.y(ThPos)==0.0d0) THEN
        y(ThPos)=y(TempPos)*(p0/y(PrePos))**(Rd/Cpd)
        WRITE(*,*) 'Berechnung Th',y(ThPos)
      END IF  
      IF (y(4)==0.0d0) THEN
        y(RhoPos)=y(1)/(Rd*y(2))
      END IF  
      IF (PRESENT(rhStart)) THEN
        ...
        rpar(5)=rhStart
        rpar(6)=rhStart
      ELSE IF (PRESENT(teStart).AND.PRESENT(qvStart)) THEN
        y(RelHPos)=QVStart*Rv*TeStart*(y(5)+y(4))/SatVap(TeStart)
      ELSE
        y(RelHPos)=0.7d0
        rpar(5)=0.7d0
        rpar(6)=0.7d0
      END IF  
      y(RhoVPos)=y(RelHPos)*SatVap(y(TempPos))/(Rv*y(TempPos))
      yPrime(1)=-y(RhoPos)*Grav
      rpar(1)=0.0d0
      rpar(2)=1.0d0
    CASE DEFAULT  
