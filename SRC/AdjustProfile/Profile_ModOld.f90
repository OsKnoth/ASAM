...
! \rho   y(4)
  ...
  Delta(4)=Te-((z-zu)*To+(zo-z)*Tu)/(zo-zu)
  Delta(5)=RhoV-Rho*QV
  Delta(6)=QV-((z-zu)*QVHo+(zo-z)*QVHu)/(zo-zu)
! WRITE(*,*) 'z     ',z
! WRITE(*,*) 'y     ',y(1:6)
! WRITE(*,*) 'yP    ',yPrime(1:6)
...
SUBROUTINE ComputeProfile(c,Height,Pre,Temp,RH,QV)
    ...
    DO  
      ...
      ELSE IF (PRESENT(QV)) THEN
        IF (rpar(5)==QV(1)) THEN
          EXIT
        END IF  
        WRITE(*,*) 'Increas QV',rpar(5),rpar(5)+1.d-4
        QVStart=MIN(rpar(5)+1.d-4,QV(1))
        CALL InputRes(y,yprime,rpar,ipar,PreStart,TeStart=TeStart,QVStart=QVStart)
      END IF  
    END DO  
  ELSE  
