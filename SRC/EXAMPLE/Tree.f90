MODULE Tree_Mod
  ...
  REAL(RealKind) :: AMP=1.0d0
  REAL(RealKind) :: alpha_x,alpha_y
...
FUNCTION uInFlow(x,y,z,Time)
  ...
  REAL(RealKind) :: U0,w,wx,wy,wz,k,knull,kmean
  INTEGER        :: s

  U0=SQRT((uMax*sin(alpha_x*Pi/180.0d0))**2+(vMax*sin(alpha_y*Pi/180.0d0))**2)
  IF (Time>1.0d0) THEN
    ...
    k_end = int(Inflow_leny/4.0)
      DO s = 1,EddySize
        phase(s)=phase(s)
        k = 20.0*((0.8)**s)
        kmean = (k/(offset_y1-offset_y))
        wy = kmean*U0

        UStart3=AMP*SIN(kmean*(y-offset_y)*2.0*Pi + phase(s)*offset_y1)* &
                    COS(Time*wy*2.0*Pi)

        uInFlow=uInFlow + UStart3
      ENDDO
    uInFlow=intenz*uInFlow
    uInFlow=uInFlow + (uMax*sin(alpha_x*Pi/180.0d0))
  ELSE
    uInFlow=uMax*sin(alpha_x*Pi/180.0d0)
  END IF

END FUNCTION uInFlow

FUNCTION vInFlow(x,y,z,Time)

  REAL(RealKind) :: vInFlow
  REAL(RealKind) :: x,y,z,Time
  ...
  ! mean wind speed adjusted according to the wind direction in x and y
  U0=SQRT((uMax*sin(alpha_x*Pi/180.0d0))**2+(vMax*sin(alpha_y*Pi/180.0d0))**2)
  ...
  IF (Time>1.0d0) THEN
    ...
    k_end = int(Inflow_leny/4.0)
      DO s = 1,EddySize
        phase(s)=phase(s)
        k = 20.0*((0.8)**s)
        kmean = (k/(offset_x1-offset_x))
        wx = kmean*U0

        VStart3=AMP*COS(kmean*(x-offset_x)*2.0*Pi + phase(s)*offset_x1)* &
                    SIN(Time*wx*2.0*Pi)

        vInFlow=vInFlow + VStart3
      END DO
    vInFlow=intenz*vInFlow
    vInFlow=vInFlow + (vMax*sin(alpha_y*Pi/180.0d0))
  ELSE
    vInFlow=vMax*sin(alpha_y*Pi/180.0d0)
  END IF

END FUNCTION vInFlow
...
FUNCTION UStart(x,y,z,zHeight,Time)
  ...
  REAL(RealKind) :: karman=0.4
  REAL(RealKind) :: x,y,z,zHeight,Time

  IF (LogWind) THEN
    UStart=((uStar/karman)*log(z/zRauh+1.0))*sin(alpha_x*Pi/180.0d0)
  ELSE IF (SineWind) THEN
    UStart=(((log((z+1.d-1)/zRauh))/(log(100.0/1d-1)))*uInFlow(x,y,z,Time))
  ELSE
    UStart=uMax*sin(alpha_x*Pi/180.0d0)
  END IF
...
FUNCTION VStart(x,y,z,zHeight,Time)
  ...
  IF (LogWind) THEN
    VStart=((uStar/karman)*log(z/zRauh+1.0))*sin(alpha_y*Pi/180.0d0)
  ELSE IF (SineWind) THEN
    VStart=(((log((z+1.d-1)/zRauh))/(log(100.0/1d-1)))*vInFlow(x,y,z,Time))
  ELSE
    VStart=uMax*cos(alpha_x*Pi/180.0d0)
  END IF
...
FUNCTION TkeStart(x,y,z,zHeight,Time)
  ...
  REAL(RealKind) :: x,y,z,zHeight,Time
  REAL(RealKind) :: c_mu=0.09
  !TkeStart=TkeMax
  TkeStart=uStar**2/sqrt(c_mu)
...
FUNCTION DisStart(x,y,z,zHeight,Time)
  ...
  REAL(RealKind) :: karman=0.4
  !DisStart=DisMax
  DisStart=uStar**3/(karman*(z+zRauh))
