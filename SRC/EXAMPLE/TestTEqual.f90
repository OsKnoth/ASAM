...
FUNCTION PressLoc(z)
  ...
  REAL(RealKind) :: z
  T0=T_freeze
  PressLoc=p0*EXP(-z/(Rd*T0))
END FUNCTION PressLoc

FUNCTION RhoFLoc(z)

  REAL(RealKind) :: RhoFLoc
  REAL(RealKind) :: z

  RhoFLoc=PressLoc(z)/(Rd*Temp0)
END FUNCTION RhoFLoc
...
FUNCTION ThetaLoc(z)
  ...
  ThetaLoc=(p0/PressLoc(z))**kappa*Temp0*RhoFLoc(z)
END FUNCTION ThetaLoc

END MODULE TestTEqual_Mod
