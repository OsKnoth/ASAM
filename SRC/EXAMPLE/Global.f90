...
1 CONTINUE
  ...
  SELECT CASE(Problem)
    ...
    CASE ('DCMIP_1_1')
      ...
      z0=1000.0d0                 ! center point in z OSSI
...
FUNCTION RhoFun(lam,phi,z,zHeight,Time)
  ...
  SELECT CASE(Problem)
    ...
    CASE ('MoistBaroIn')
      ...
      RhoFun=pLoc/(ThLoc*((Rv-Rd)*qVLoc+Rd))
    CASE ('William6')
...
FUNCTION UStart(lam,phi,z,zHeight,Time)
  ...
  SELECT CASE(Problem)
    ...
    CASE('GlobalGravity')
      UStart=U0*COS(phi)
    CASE('BaroIn','MoistBaroIn','DCMIP_4_1')
      CALL Rotate(lam,phi,rot_lam,rot_phi)
      eta=eta_n1(rot_lam,rot_phi,z,ABS(U0))
