...
1 CONTINUE
  ...
  IF (Shallow) THEN
    ...
    delta=Two
  ELSE
    Const=p0*(Rd*th0/p0)**(One/(One-Kappa))
    delta=One/(One-Kappa)
  END IF
...
FUNCTION RhoFun(x,y,z,zHeight,Time)
  ...
  RhoFun=Rho0*(One+Ampli*SIN(Two*Pi*z/Lenz))
...
FUNCTION WStart(x,y,z,zHeight,Time)
  ...
  RhoFun=Rho0*(One+Ampli*SIN(Two*Pi*z/Lenz))
  WStart=Two*SQRT(Const*delta)/(delta-One)*RhoFun**((delta-one)/Two) &
        -Two*SQRT(Const*delta)/(delta-One)*Rho0**((delta-one)/Two)
END FUNCTION WStart
