MODULE Real_Mod
  ...
  REAL(RealKind) :: zShear=10000.0d0
  REAL(RealKind) :: TkeHMax=1.0d-2
  REAL(RealKind) :: TkeVMax=1.0d-2
  REAL(RealKind) :: LenMax=1.0d0
  REAL(RealKind) :: AeroStart=300.0d6  ! N_CCN at 1% supersaturation
  REAL(RealKind) :: HeightTracer1Top=700.0d0      ! MBL tracer top
  REAL(RealKind) :: HeightTracer1Bottom=0.0d0     ! MBL tracer bottom
  REAL(RealKind) :: HeightTracer2Top=2250.0d0     ! SD  tracer top
  REAL(RealKind) :: HeightTracer2Bottom=1750.0d0  ! SD tracer bottom
  REAL(RealKind) :: DeltaT=0.02d0           ! T-Perturbation for periodic domain
  REAL(RealKind) :: HeightDeltaT=1000.0d0
  REAL(RealKind) :: DeltaQv=0.0d-3         ! qv-Perturbation for periodic domain
  REAL(RealKind) :: HeightDeltaQv=1000.0d0
  REAL(RealKind) :: SST=300.65d0  ! summer: 300.65 K, winter: 299.15 K
  LOGICAL :: ProfIn=.FALSE.
  LOGICAL :: Perturb=.FALSE.
  LOGICAL :: BomexWind=.FALSE.
  CHARACTER*20 :: CaseName='Barbados'
...
SUBROUTINE SetBoundCells(BoundCellLoc)
  ...
  BoundCellLoc%TeS=SST   ! summer: 300.65 K, winter: 299.15 K
...
FUNCTION ForceRho(x,y,z,zHeight,Time)
  ...
  CASE('Barbados') ! BOMEX large-scale drying
    IF (z<300.0d0.AND.z-zHeight>Zero) THEN
      ForceRho=-1.2d-8 
    ELSE IF (z>=300.0d0.AND.z<500.0d0.AND.z-zHeight>Zero) THEN
...
FUNCTION ForceTh(x,y,z,zHeight,Time)
  ...
  REAL(RealKind) :: x,y,z,zHeight,Time
  SELECT CASE(CaseName)
  CASE('Barbados') ! BOMEX large-scale cooling
    IF (z<1500.0d0.AND.z-zHeight>Zero) THEN
      ForceTh=-2.135d-5
    ELSE IF (z>=1500.0d0.AND.z<2500.0d0.AND.z-zHeight>Zero) THEN
