MODULE Tree_Mod
  ...
  REAL(RealKind) :: alpha_x
  REAL(RealKind) :: alpha_y
      ...
      DO s = 1,10 !k_end
        k = ((10.0)*(0.9**s)) !*(1.0 + ((- 0.5d-1 + 1.0d-1*r)/((6.0*(0.8**1.0)))))
          DO ss = 1,2
            ...
            VStart3 = AMP*SIN(k*((x-offset_x)/(offset_x1-offset_x))*2.0*3.1415 + &
                  Time*(wx)*2.0*3.1415 + k**(2.0))  &
                + AMP*SIN(k*((y-offset_y)/(offset_y1-offset_y))*2.0*3.1415 + &
                  Time*(wy)*2.0*3.1415 + k**(2.0))  &
                + AMP*SIN(k*((z-offset_z)/(offset_z1-offset_z))*2.0*3.1415 + &
          ...
          ENDDO
      ENDDO
    vInFlow=(intenz*(SQRT((uMax**2.0)+(vMax**2.0)))*vInFlow/((s-1)*(ss-1))) + vMax
  ELSE
    vInFlow=VMax
  END IF
...
!    UStart=((log((z+1.d-1)/zRauh))/(log(100.0/1d-1)))*uMax !*uMax
    UStart=((uStar/karman)*log(z/zRauh+1.0))*alpha_x !uMax
...
FUNCTION VStart(x,y,z,zHeight,Time)
  ...
  IF (LogWind) THEN
    VStart=((uStar/karman)*log(z/zRauh+1.0))*alpha_y
!    VStart=((log((z+1.d-1)/zRauh))/(log(100.0/1d-1)))*vMax 
  ELSE IF (SineWind) THEN
