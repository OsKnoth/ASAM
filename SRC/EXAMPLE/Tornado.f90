MODULE Tornado_Mod
  ...
  REAL(RealKind) :: Th0=250.0d0
  REAL(RealKind) :: N=1.d-2
  REAL(RealKind) :: NL=1.0d-2
  REAL(RealKind) :: NU=1.0d-2
  REAL(RealKind) :: H=1.0d-2
  REAL(RealKind) :: x0Tornado=100.0d0
  REAL(RealKind) :: x1Tornado=500.0d0
  REAL(RealKind) :: x0Force=1000.0d0
  REAL(RealKind) :: x1Force=2000.0d0
  REAL(RealKind) :: y0Force=1000.0d0
  REAL(RealKind) :: y1Force=2000.0d0
  REAL(RealKind) :: z0Force=100.0d0
  REAL(RealKind) :: z1Force=500.0d0
  REAL(RealKind) :: ForceRate=0.01d0
...
SUBROUTINE SetBoundCells(BoundCellLoc)
  ...
  BoundCellLoc%TeS=300.d0
  BoundCellLoc%ThetaS=300.0d0
  BoundCellLoc%LandClass=1
  BoundCellLoc%zRauh=0.1d0
  BoundCellLoc%zRauhT=0.1d0
...
FUNCTION ForceTh(x,y,z,zHeight,Time)
  ...
  REAL(RealKind) :: x,y,z,zHeight,Time
  IF (x>=x0Force.AND.x<=x1Force.AND. &    
      y>=y0Force.AND.y<=y1Force.AND. &
      z>=z0Force.AND.z<=z1Force) THEN
    ForceTh=ForceRate
  ELSE
    ForceTh=0.0d0
  END IF  
