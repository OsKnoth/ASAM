...
FUNCTION UStart(x,y,z,zHeight,Time)
  ...
  REAL(RealKind) :: U0,w,w2,k,knull,AMP=1.0
  INTEGER        :: ss,s
...
!  //////turb = ran1()
  ...
  DO s = 1,2! !k_end
     k = ((k_end)*(0.97**s)) !*(1.0 + ((- 0.5d-1 + 1.0d-1*r)/((6.0*(0.8**1.0))))) !40.0*(0.97)**s
     k = 5.0
      DO ss = 1,2
!     w = ((-1.0)**ss)*k*(SQRT(uMax))/Inflow_leny 
     w = ((-1.0)**ss)*k*(SQRT(uMax**Two+vMax**Two))/Inflow_leny 
     w2 = ((-1.0)**ss)*k*(SQRT(uMax**Two))/Inflow_leny-&
((-1.0)**ss)*k*(SQRT(vMax**Two))/Inflow_leny 
...
ELSE
     UStart3 = AMP*SIN(k*((y-offset_y)/Inflow_leny)*2.0*3.1415 + Time*(w2)*2.0*3.1415 )!+ 3.14/Two) !k**(2.0))  &!- 0.5d-1 + 1.0d-1*r  
!             + (AMP/1.0)*SIN(k*((z)/Inflow_len)*2.0*3.1415 + Time*(w)*2.0*3.1415 + k**(2.0))! - 0.5d-2 + 1.0d-2*r)) 
...
FUNCTION VStart(x,y,z,zHeight,Time)
  ...
  REAL(RealKind) :: x,y,z,zHeight,Time
  REAL(RealKind) :: yL,yR,r
  REAL(RealKind) :: V0,w,k,knull,AMP=1.0
  INTEGER        :: ss,s
  CALL Random_Number(r)
  VStart=0.0d0
  knull=18.0
  k_end =  int(Inflow_lenx/4.0)
  DO s = 1,1 !k_end !100
     k = ((k_end)*(0.97**s))  !- 0.5d-1 + 1.0d-1*r ! *(1.0 + ((- 0.5d-1 + 1.0d-1*r)/((s)))) !40.0*(0.97)**s
     k = 5.0
    DO ss = 1,2
!     w = ((-1.0)**ss)*k*(vMax)/Inflow_lenx
     w = ((-1.0)**ss)*k*(SQRT(uMax**Two+vMax**Two))/Inflow_lenx
  AMP=(((k**4.0)*(exp(-2.0*(k/knull)**2.0)))/((knull**4.0)*(exp(-2.0*(knull/knull)**2.0))))!**(1.0/10.0)
!  VStart3=((((8.0/3.14)*(1.0**2.0)*(((k/knull)**4.0)/(knull*(1.0+(k/knull)**2.0)**3.0))/ & 
!           (4.0*3.14*k**2.0)))**(1.0/2.0)) &
!               *COS(k*((y-offset_y)/Inflow_len)*2.0*3.1415 + Time*(w)*2.0*3.1415 + k**(2.0 - 0.5d-1 + 1.0d-1*r)) &
IF(x.lt.2.and.y.lt.Inflow_leny) THEN
VStart2 = 1.0*AMP*COS(k*((y-offset_y)/Inflow_leny)*2.0*3.1415 +  Time*(-w)*2.0*3.1415 ) !- 0.5d-1 + 1.0d-1*r  
VStart=VStart + VStart2
ELSE
      VStart3 = AMP*COS(k*((x-offset_x)/Inflow_lenx)*2.0*3.1415 + Time*(w)*2.0*3.1415 + 3.14/Two) !k**(2.0)) ! - 0.5d-1 + 1.0d-1*r 
!               AMP*COS(k*((y-offset_y)/Inflow_leny)*2.0*3.1415 + Time*(w)*2.0*3.1415 + k**(2.0))  ! - 0.5d-1 + 1.0d-1*r 
...
!  IF(y.LT.(offset_y1)) VStart = vMax
!  VStart=(((0.44d0/0.4d0)*(log(z/0.1d0)))/((0.44d0/0.4d0)*(log(16.0/0.1d0))))*VStart - 0.5d-1 + 1.0d-1*r
!  VStart=(((0.44d0/0.4d0)*(log(z/0.1d0)))/((0.44d0/0.4d0)*(log(16.0/0.1d0))))*vMax - 0.5d-1 + 1.0d-1*r
  VStart = vMax
!  VStart = vMax - 0.5d-1 + r*0.1d0
!  VStart = vMax - 0.5d0 + r
END FUNCTION VStart
