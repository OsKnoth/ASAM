MODULE DomainDecomposition_Mod
  ...
  REAL(RealKind) :: xMax=2000.0d0
  REAL(RealKind) :: xLen=1000.0d0
  REAL(RealKind) :: yMax=2000.0d0
  REAL(RealKind) :: yLen=1000.0d0
  REAL(RealKind) :: zMax=2000.0d0
  REAL(RealKind) :: zLen=2000.0d0
...
END MODULE DomainDecomposition_Mod


SUBROUTINE SetBoundCells(BoundCellLoc)

  USE DomainDecomposition_Mod
  USE DataType_Mod
  IMPLICIT NONE
...
FUNCTION UStart(x,y,z,zHeight,Time)
  ...
  SELECT CASE(Plane)
    CASE('XZ')
     IF (ABS(x-xMax)<=xLen.AND.ABS(z-zMax)<=zLen) THEN
       UStart=UStart+vPert
     END IF  
    CASE('XY')
     IF (ABS(x-xMax)<=xLen.AND.ABS(y-yMax)<=yLen) THEN
       UStart=UStart+vPert
     END IF  
  END SELECT  
...
FUNCTION VStart(x,y,z,zHeight,Time)
  ...
  SELECT CASE(Plane)
    CASE('YZ')
     IF (ABS(y-yMax)<=yLen.AND.ABS(z-zMax)<=zLen) THEN
       VStart=VStart+vPert
     END IF  
    CASE('XY')
     IF (ABS(x-xMax)<=xLen.AND.ABS(y-yMax)<=yLen) THEN
