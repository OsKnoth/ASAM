MODULE Grid_Mod
  ...
  !INTEGER, PARAMETER :: IncrVol=10
  !REAL(8), PARAMETER :: Shrink=1.0d-6
  ! INTEGER, PARAMETER :: IncrVol=1    ! wenn EinheitswürfelRechnung
  ! REAL(8), PARAMETER :: Shrink=0.0   ! wenn EinheitswürfelRechnung
  !REAL(8) :: dxViewLoc=1.0d-8
  !REAL(8) :: dyViewLoc=1.0d-8
  !REAL(8) :: dzViewLoc=1.0d-8
  !REAL(8) :: dxLoc=0.0d0
  !REAL(8) :: dyLoc=0.0d0
  !REAL(8) :: dzLoc=0.0d0
...
SUBROUTINE SetWeightBorders 
!
!---------------------------------------------------------------
!--- Set Weight Borders all Blocks 
!---------------------------------------------------------------
    INTEGER :: ib, ib1, in
    ...
    DO ib=1,nb
      ...
      DO in=1,AnzahlNachbar
        ib1=Nachbars(in)%ib
        SELECT CASE  (Nachbars(in)%nType)
          CASE ('iw')
             dx(ix0)=Floor(ib1)%dx(Floor(ib1)%ix1)
             xP(ix0-1)=Floor(ib1)%xP(Floor(ib1)%ix1-1)
          CASE ('ie')
             dx(ix1+1)=Floor(ib1)%dx(Floor(ib1)%ix0+1)
             xP(ix1+1)=Floor(ib1)%xP(Floor(ib1)%ix0+1)
          CASE ('is')
             dy(iy0)=Floor(ib1)%dy(Floor(ib1)%iy1)
             yP(iy0-1)=Floor(ib1)%yP(Floor(ib1)%iy1-1)
          CASE ('in')
             dy(iy1+1)=Floor(ib1)%dy(Floor(ib1)%iy0+1)
             yP(iy1+1)=Floor(ib1)%yP(Floor(ib1)%iy0+1)
          CASE ('ib')
             dz(iz0)=Floor(ib1)%dz(Floor(ib1)%iz1)
             zP(iz0-1)=Floor(ib1)%zP(Floor(ib1)%iz1-1)
          CASE ('it')
             dz(iz1+1)=Floor(ib1)%dz(Floor(ib1)%iz0+1)
             zP(iz1+1)=Floor(ib1)%zP(Floor(ib1)%iz0+1)
        END SELECT
      END DO      ! in
    END DO         ! ib

END SUBROUTINE SetWeightBorders 
...
!   Read data from file
  ...
  DO
    ...
    !--------------------------------------------
       ...
       WRITE(*,*)
       WRITE(*,*) 'Selected RadOutput : ',RadOutput, '  (value to numerator for parametrization)'
       WRITE(*,*) 'Selected ScaleRad  : ',ScaleRad, '  (value to denominator for parametrization)'
       WRITE(*,*) 'Selected ScaleSoil : ',ScaleSoil, ' (value to scale dzi_soil in %)'
       WRITE(*,*)
    ...
    ELSE IF (INDEX(Line,'#Funktion')>0) THEN
    !---------------------------------------
       nr_wahlfkt=0
2      CONTINUE
       READ(InputUnit,*) read_namefkt
       name_fkt=ADJUSTL(read_namefkt)
       WRITE(*,*)
       WRITE(*,*) "Function : ",TRIM(name_fkt)
       IF (nr_wahlfkt==0) THEN
         DO i=1,anz_fkt
           IF(w_namefkt(i)==TRIM(name_fkt)) THEN
             nr_wahlfkt=i
           END IF
         END DO
       ELSE IF (nr_wahlfkt==nr_fkt_oro) THEN
         DO i=1,anz_fkt
           IF(w_namefkt(i)==TRIM(name_fkt)) THEN
             IF (i==nr_fkt_gml) THEN
               nr_wahlfkt=nr_fkt_gml_oro
             ELSE
               WRITE(*,*) '  Not considered / Orography only'
             END IF
           END IF
         END DO
         IF (nr_wahlfkt==nr_fkt_oro) GOTO 3
       END IF
       IF(nr_wahlfkt==nr_fkt_haus) THEN          ! Haus
         READ(InputUnit,*) read_namefkt
         file_namefkt=ADJUSTL(read_namefkt)
         WRITE(*,*) '  ',file_namefkt
       ELSE IF(nr_wahlfkt==nr_fkt_gml    .OR.  & ! GML
               nr_wahlfkt==nr_fkt_gml_oro ) THEN ! GML + Oro
         READ(InputUnit,*) read_namefkt
         file_namefkt=ADJUSTL(read_namefkt)
         WRITE(*,*) '  ',file_namefkt
       ELSE IF(nr_wahlfkt==nr_fkt_oro) THEN      ! Oro
         READ(InputUnit,*) read_namefkt
         file_namefkt=ADJUSTL(read_namefkt)
         WRITE(*,*) '  ',file_namefkt
       ELSE IF(nr_wahlfkt==nr_fkt_sierra) THEN   ! Curfit_approximation
         READ(InputUnit,*) read_namefkt
         file_namefkt=ADJUSTL(read_namefkt)
         WRITE(*,*) '  ',file_namefkt
       ELSE IF(nr_wahlfkt==nr_fkt_surfit) THEN   ! Surfit-approximation
         READ(InputUnit,*) read_namefkt
         file_namefkt=ADJUSTL(read_namefkt)
         WRITE(*,*) '  ',file_namefkt
       ELSE IF(nr_wahlfkt==nr_fkt_wgs_oro) THEN  ! WGS84_Oro
         READ(InputUnit,*) read_namefkt
         file_namefkt=ADJUSTL(read_namefkt)
         WRITE(*,*) '  ',file_namefkt
       ELSE IF(nr_wahlfkt==nr_fkt_wgs_surf) THEN ! WGS84_Surf
         READ(InputUnit,*) read_namefkt
         file_namefkt=ADJUSTL(read_namefkt)
         WRITE(*,*) '  ',file_namefkt
       END IF
       CALL SelectReadFkt(nr_wahlfkt)
       !WRITE(*,*)
       !WRITE(*,*) "Function : ",TRIM(name_fkt), "   ", TRIM(NameFun)
       !WRITE(*,*) "---------- ",("-",i=1,LEN_TRIM(name_fkt)),("-",i=1,LEN_TRIM(NameFun))
       !WRITE(*,*) "           ",TRIM(file_namefkt)
       IF (nr_wahlfkt==9) THEN
          WRITE(*,*) "           ",TRIM(NameFun)
          WRITE(*,*) "           ",TRIM(file_namefkt)
       END IF
       IF (nr_wahlfkt==nr_fkt_oro) GOTO 2
3      CONTINUE
       IF (nr_wahlfkt==nr_fkt_gml_oro) WRITE(*,*) '--> Function: Haus_GML + Oro'
       WRITE(*,*)

    ELSE IF (INDEX(Line,'#Emission')>0) THEN
      READ(InputUnit,*) read_nameemi
      name_emi=ADJUSTL(read_nameemi)
      WRITE(*,*) "Emission: ",TRIM(name_emi)
      DO i=1,anz_emi
        IF(w_nameemi(i)==TRIM(name_emi)) THEN
          nr_wahlemi=i
        END IF
      END DO
      IF( (nr_wahlemi==1)             .AND. &
          (nr_wahlfkt==nr_fkt_oro     .OR.  &
           nr_wahlfkt==nr_fkt_gml_oro) ) THEN
        nr_wahlemi=nr_emi_street_oro
      END IF
      IF (nr_wahlemi>0) THEN
        READ(InputUnit,*) read_nameemi
        file_nameemi=ADJUSTL(read_nameemi)
        WRITE(*,*) '  ',file_nameemi
        CALL ReadEmission
      ELSE
        WRITE(*,*) '  No emission'
      END IF
      IF (nr_wahlemi==nr_emi_street_oro) WRITE(*,*) '--> Emission: Emi_Street + Oro'
      WRITE(*,*)

    ELSE IF (INDEX(Line,'#Masking')>0) THEN
      READ(InputUnit,*) read_namemask
      name_mask=ADJUSTL(read_namemask)
      WRITE(*,*) "Masking:  ",TRIM(name_mask)
      DO i=1,anz_mask
        IF(w_namemask(i)==TRIM(name_mask)) THEN
          nr_wahlmask=i
        END IF
      END DO
      IF( (nr_wahlmask==1)            .AND. &
          (nr_wahlfkt==nr_fkt_oro     .OR.  &
           nr_wahlfkt==nr_fkt_gml_oro) ) THEN
        nr_wahlmask=nr_mask_baum_oro
      END IF
      IF (nr_wahlmask>0) THEN
        READ(InputUnit,*) read_namemask
        file_namemask=ADJUSTL(read_namemask)
        WRITE(*,*) '  ',file_namemask
        CALL ReadMasking
      ELSE
        WRITE(*,*) '  No masking'
      END IF
      IF (nr_wahlmask==nr_mask_baum_oro) WRITE(*,*) '--> Masking: Mask_Baum + Oro'
      WRITE(*,*)
    ...
    ELSE IF (INDEX(Line,'#Multiblock')>0) THEN
    !-----------------------------------------
       Domain%ix0=0
       Domain%iy0=0
       Domain%iz0=0
 ...
 !     !Block structure
    ...
    !----------------------------------------
       ...
       DO ib=1,nr_sb
         ...
         ALLOCATE(Floor(id_sb)%soil_type(1:nr_soildef,1:8))
         ...
         DO i=1,nr_soildef
            ...
            READ(InputUnit,*) (Floor(id_sb)%soil_type(i,j), j=1,7) 
         END DO
       END DO  ! nr_sb
    ...
    !-------------------------------------------
       IF(nr_wahlfkt==nr_fkt_wgs_oro .OR. &
          nr_wahlfkt==nr_fkt_wgs_surf) THEN
           ...
           ifcorine=.TRUE.
           struct_bound=struct_bound+1 
           nr_landdef=Nr_Land_Cl  !symbolisch
       ELSE 
          READ(InputUnit,*) nr_landdef
          if( nr_landdef>0) then
             IF (nr_sb>0) THEN
                struct_bound=struct_bound+1 !struct_bound=5
             ELSE
                struct_bound=struct_bound+3 !struct_bound=6
             END IF
  ...
  END DO
1 CONTINUE
  IF (nr_wahlfkt==-1) THEN   ! def. Standard
    nr_wahlfkt=0
    CALL SelectReadFkt(nr_wahlfkt)
  END IF
...
SUBROUTINE read_grid_weight(FileName)
...
!   Read data from file
  ...
  DO
    ...
    ELSE IF (INDEX(Line,'&OutGMVControl')>0) THEN
      ...
      IF(TRIM(indata_type) == gausskrueger ) THEN
        ...
        ELSE
          ...
          Write(*,*) 'zero point scale put off ---> lower edge domain'
        END IF
      ELSE IF (TRIM(indata_type)==geographical) THEN
        Write(*,*) 'Geographisch:  '
        Write(*,*) '  Domain%x0=', Domain%x0, '  Domain%y0=', Domain%y0
        Write(*,*) '  Domain%x1=', Domain%x1, '  Domain%y1=', Domain%y1
        Domain%x0=(Domain%x0*4.0d0*ATAN(1.0d0))/180
        Domain%x1=(Domain%x1*4.0d0*ATAN(1.0d0))/180
        Domain%y0=(Domain%y0*4.0d0*ATAN(1.0d0))/180
        Domain%y1=(Domain%y1*4.0d0*ATAN(1.0d0))/180
        Write(*,*) 'Bogenmass:'
        Write(*,*) '  Domain%x0=', Domain%x0, '  Domain%y0=', Domain%y0
        Write(*,*) '  Domain%x1=', Domain%x1, '  Domain%y1=', Domain%y1
      ELSE IF (TRIM(indata_type)=='cyl') THEN
    ...
    ELSE IF (INDEX(Line,'#Funktion')>0) THEN
      ...
      DO i=1,anz_fkt
        ...
        END IF
      END DO
      IF(nr_wahlfkt==nr_fkt_haus) THEN
        READ(InputUnit,*) read_namefkt
        file_namefkt=ADJUSTL(read_namefkt)
      ELSE IF(nr_wahlfkt==nr_fkt_gml) THEN
        READ(InputUnit,*) read_namefkt
        file_namefkt=ADJUSTL(read_namefkt)
      ELSE IF(nr_wahlfkt==nr_fkt_oro) THEN
        READ(InputUnit,*) read_namefkt
        file_namefkt=ADJUSTL(read_namefkt)
      ELSE IF(nr_wahlfkt==nr_fkt_sierra) THEN
        READ(InputUnit,*) read_namefkt
        file_namefkt=ADJUSTL(read_namefkt)
      ELSE IF(nr_wahlfkt==nr_fkt_surfit) THEN
        READ(InputUnit,*) read_namefkt
        file_namefkt=ADJUSTL(read_namefkt)
      END IF
      WRITE(*,*)
    ...
    ELSE IF (INDEX(Line,'#Multiblock')>0) THEN
    !ELSE IF (INDEX(Line,'#Weight')>0) THEN
      Domain%ix0=0
      Domain%iy0=0
      Domain%iz0=0
      READ(InputUnit,*) Domain%ix0,Domain%ix1
      ...
      !IF(out_area=='n') THEN  !nur im Weight-Teil kommentieren
      !                        !Domain%view_*** wird domain%**View zugewiesen und in 
      !                        !CheckVertex und AnalyzeEdge verwendet
  ...
  !   Block structure
      ...
      DO ib=1,nb
       ...
       !  READ(InputUnit,*) Floor(ib)%igz0,Floor(ib)%igz1
        ...
        ! READ(InputUnit,*) Floor(ib)%Refine,Floor(ib)%RefineX,Floor(ib)%RefineY,Floor(ib)%RefineZ,Floor(ib)%RefLevel
         ...
         Floor(ib)%igx0=Domain%ix0
         Floor(ib)%igx1=Domain%ix1
         Floor(ib)%igy0=Domain%iy0
         Floor(ib)%igy1=Domain%iy1
         Floor(ib)%igz0=Domain%iz0
         Floor(ib)%igz1=Domain%iz1
         Floor(ib)%Refine=0
         Floor(ib)%RefineX=0
         Floor(ib)%RefineY=0
         Floor(ib)%RefineZ=0
      ...
      IF (nr_wahlfkt==0) THEN   ! def. Standard
        CALL SelectReadFkt(nr_wahlfkt)
      END IF
...
1 CONTINUE
 ...
 END SUBROUTINE read_grid_weight
...
SUBROUTINE InitAllVertices
  ...
  DO ib=1,nb
    ...
    DO i=ix0,ix1
      DO j=iy0,iy1
        DO k=iz0,iz1
           Vertices(i,j,k)%Point%x=xP(i)
           Vertices(i,j,k)%Point%y=yP(j)
           Vertices(i,j,k)%Point%z=zP(k)
           Vertices(i,j,k)%in_out=999
           Vertices(i,j,k)%nrP=-1
           Vertices(i,j,k)%nrInP=-1
           Vertices(i,j,k)%nrCutP=-1
        END DO
      END DO
    END DO
    ! for block borders 
    ! Variante Rand -> %nr[P,InP,CutP) mit -99 belegt
    widthx=((ix1+1)-(ix0-1))
    widthy=((iy1+1)-(iy0-1))
    widthz=((iz1+1)-(iz0-1))
    !east,west
    DO i=ix0-1,ix1+1,widthx
      DO j=iy0-1,iy1+1  !j=iy0,iy1
        DO k=iz0-1,iz1+1  !k=iz0,iz1
           Vertices(i,j,k)%Point%x=xP(i)
           Vertices(i,j,k)%Point%y=yP(j)
           Vertices(i,j,k)%Point%z=zP(k)
           Vertices(i,j,k)%in_out=999
           Vertices(i,j,k)%nrP=-99
           Vertices(i,j,k)%nrInP=-99
           Vertices(i,j,k)%nrCutP=-99
        END DO
      END DO
    END DO
    !top,bottom
...
SUBROUTINE AnalyzeAllVertices
  ...
  INTEGER :: ListP(3,1000),NumList,iNum,in_out
  ...
  !dxLoc=(Domain%x1-Domain%x0)/Domain%nx*0.05d0  ! bei Surfit zu gross
  !dyLoc=(Domain%y1-Domain%y0)/Domain%ny*0.05d0
  !dzLoc=(Domain%z1-Domain%z0)/Domain%nz*0.05d0
  !dxLoc=(Domain%x1-Domain%x0)/Domain%nx*0.01d0   ! bei Surfit okey
  !dyLoc=(Domain%y1-Domain%y0)/Domain%ny*0.01d0
  !dzLoc=(Domain%z1-Domain%z0)/Domain%nz*0.01d0
...
! Suche Schnittpunkte
  DO ib=1,nb
    ...
    DO i=ix0,ix1
      DO j=iy0+1,iy1
        DO k=iz0,iz1
    !IF(i==74.and.j==3.and.k==12) THEN
    !   write(*,*) "AnalyzeEdge(Edges_Y(i,j,k)%Edge) (74,3,12), Kinzig,v893712"
    !END IF
    ...
    !IF(i==74.and.j==3.and.k==12) THEN
    !   write(*,*) "AnalyzeEdge(Edges_Z(i,j,k)%Edge) (74,3,12), Kinzig,v893712"
    !END IF

            CALL AnalyzeEdge(Edges_Z(i,j,k)%Edge)

!if(i==1 .AND. j==80 .AND. k==5) then   !Celle 2,81,5
!     Write(*,*)  "Vert1", Edges_Z(i,j,k)%Edge%Vert1%Point
!     Write(*,*)  "Vert2", Edges_Z(i,j,k)%Edge%Vert2%Point
!     Write(*,*)  "VertS", Edges_Z(i,j,k)%Edge%VertS%Point
!end if
!
!if(Edges_Z(i,j,k)%Edge%Vert1%in_out==0 .AND. Edges_Z(i,j,k)%Edge%Vert2%in_out==1) then
!     Write(OutUnitProt,*) "Edges_Z(i,j,k)%Edge%Vert1%in_out==0 .AND."
!     Write(OutUnitProt,*) "Edges_Z(i,j,k)%Edge%Vert2%in_out==1)"
!     Write(OutUnitProt,*) "i=",i,"  j=",j, "  k=",k
!     Write(OutUnitProt,*)  "Vert1", Edges_Z(i,j,k)%Edge%Vert1%Point
!     Write(OutUnitProt,*)  "Vert2", Edges_Z(i,j,k)%Edge%Vert2%Point
!    if(ASSOCIATED(Edges_Z(i,j,k)%Edge%VertS) ) then
!     Write(OutUnitProt,*)  "VertS", Edges_Z(i,j,k)%Edge%VertS%Point
!    end if
!end if
!if(Edges_Z(i,j,k)%Edge%Vert1%in_out==-1 .AND. Edges_Z(i,j,k)%Edge%Vert2%in_out==0) then
!     Write(OutUnitProt,*) "Edges_Z(i,j,k)%Edge%Vert1%in_out==-1 .AND."
!     Write(OutUnitProt,*) "Edges_Z(i,j,k)%Edge%Vert2%in_out==0"
!     Write(OutUnitProt,*) "i=",i,"  j=",j, "  k=",k
!     Write(OutUnitProt,*)  "Vert1", Edges_Z(i,j,k)%Edge%Vert1%Point
!     Write(OutUnitProt,*)  "Vert2", Edges_Z(i,j,k)%Edge%Vert2%Point
!    if (ASSOCIATED(Edges_Z(i,j,k)%Edge%VertS)) then
...
SUBROUTINE AnalyzeAllFaces
  ...
  DO ib=1,nb
    ...
    DO i=ix0+1,ix1
      DO j=iy0+1,iy1
        DO k=iz0,iz1
          ...
          IF(i==5.and.j==40.and.k==38) Then   ! Soil Analyze
             Write(*,*) "i==5.and.j==40.and.k==38"
          END IF
    ...
    DO i=ix0,ix1
      DO j=iy0+1,iy1
        DO k=iz0+1,iz1
! IF(ib==2.AND.i==0.AND.j==75.AND.k==3) THEN
!   Write(*,*) "i==0 j==75 k==3", " AnalyzeFaceYZ", " Test:  Valley3D_4B_Osw.grid  -Version:8.9.2.3"
! END IF
    ...
    DO i=ix0+1,ix1
      DO j=iy0,iy1
        DO k=iz0+1,iz1
 !IF(ib==2.AND.i==8.AND.j==99.AND.k==1) THEN
 !  Write(*,*) "i==8 j==99 k==1", " AnalyzeFaceZX", " Test:  Valley3D_4B_Osw.grid  -Version:8.9.2.3"
 !END IF
...
SUBROUTINE EvalBorderFaceYZ_WE(ib,ix,vix,iy,iz,ibn,vjx,fac)
  ...
  !    P0----------P1
  !.........................................
  !Zuweisung Face-Rand-YZ (WE) für Übergabe (vix+fac) Ebene
  FaceRa%V0=Floor(ib)%Vertices(vix+fac,iy-1,iz-1)
  FaceRa%V1=Floor(ib)%Vertices(vix+fac,iy  ,iz-1)
  FaceRa%V2=Floor(ib)%Vertices(vix+fac,iy-1,iz)
  FaceRa%V3=Floor(ib)%Vertices(vix+fac,iy  ,iz)
  !.........................................
  !Orientierung Edge1-Edge4 in F3 u. F4 verschieden als F1,F2,F5,F6
  FaceRa%Edge1%in_out=FaceRa%V0%in_out+FaceRa%V1%in_out
        ...
        DO k=iz0+1,iz1
          IF (ASSOCIATED(Cell(i,j,k)%Cell)) THEN
             ...
             IF (Cell(i,j,k)%Cell%vc>0) THEN
               ...
               IF(k==1.AND. (Cell(i,j,k)%Cell%Face1%Edge1%yes_sp==1.OR. &
  ...
  IF ((Cell%in_out<6.AND.Cell%in_out>-8.AND.Cell%Vol>0.0d0) .OR. &
    ...
    ELSE IF (Cell%V0%in_out==0.AND.Cell%V1%in_out==0.AND.Cell%V2%in_out==0.AND.Cell%V3%in_out==0.AND. &
       ...
       !IF (Cell%V0%Point%z==0.0d0 ) THEN
          ...
          Cell%Vol=ABS(VolP8(P0,P1,P2,P3,P4,P5,P6,P7))
          Cell%MidPoint=VolMidP8(P0,P1,P2,P3,P4,P5,P6,P7)
          NrMP_Cells=NrMP_Cells+1
          Cell%mp=NrMP_Cells
...
SUBROUTINE EvalBordCell_WE(ib,ix,vix,iy,iz,ibn,vjx,fac)
  ...
  TYPE(CellP8_T) :: CellRa
     ...
     if(fac==-1) THEN !west-direction
       CellRa%V0=Floor(ib)%Vertices(vix+fac,iy-1,iz-1) !V-P1
       CellRa%V1=Floor(ib)%Vertices(vix    ,iy-1,iz-1) !V-P0
       CellRa%V2=Floor(ib)%Vertices(vix+fac,iy  ,iz-1) !V-P3
       CellRa%V3=Floor(ib)%Vertices(vix    ,iy  ,iz-1) !V-P2
       CellRa%V4=Floor(ib)%Vertices(vix+fac,iy-1,iz)   !V-P5
       CellRa%V5=Floor(ib)%Vertices(vix    ,iy-1,iz)   !V-P4
       CellRa%V6=Floor(ib)%Vertices(vix+fac,iy  ,iz)   !V-P7
       CellRa%V7=Floor(ib)%Vertices(vix    ,iy  ,iz)   !V-P6
     else if(fac==1) THEN !east-direction
       CellRa%V0=Floor(ib)%Vertices(vix    ,iy-1,iz-1) !V-P0
       CellRa%V1=Floor(ib)%Vertices(vix+fac,iy-1,iz-1) !V-P1
       CellRa%V2=Floor(ib)%Vertices(vix    ,iy  ,iz-1) !V-P2
       CellRa%V3=Floor(ib)%Vertices(vix+fac,iy  ,iz-1) !V-P3
       CellRa%V4=Floor(ib)%Vertices(vix    ,iy-1,iz)   !V-P4
       CellRa%V5=Floor(ib)%Vertices(vix+fac,iy-1,iz)   !V-P5
       CellRa%V6=Floor(ib)%Vertices(vix    ,iy  ,iz)   !V-P6
       CellRa%V7=Floor(ib)%Vertices(vix+fac,iy  ,iz)   !V-P7
     end if
     !.........................................
     CALL AnalyzeCellRand(CellRa)
     !if(ix==10.AND.iy==6.AND.iz==1) then
     ! !ContainerT_Versch_2Blk.grid ; V 8.9.3.1
     ! Write(*,*) 
     ! Write(*,*) "Check : "
     ! Write(*,*) "CellRa%Vol ix= 10 iy= 6 iz= 1"
     ! Write(*,*) "Cell%Vol=",CellRa%Vol
     ! Write(*,*) 
     !end if
     !if(ix==10.AND.iy==6.AND.iz==2) then
     ! !ContainerT_Versch_2Blk.grid ; V 8.9.3.1
     ! Write(*,*) 
     ! Write(*,*) "Check : "
     ! Write(*,*) "CellRa%Vol ix= 10 iy= 6 iz= 2"
     ! Write(*,*) "Cell%Vol=",CellRa%Vol
     ! Write(*,*) 
     !end if
     !IF(CellRa%in_out>-8.AND.CellRa%in_out<6) THEN  ?NrMP nein NrW ja?
     !IF(CellRa%in_out>-8.AND.!(CellRa%in_out==6.AND.vc==0)
     ...
     !!1 !getestet 26.11.2009,VGrid_GMV_8.4.1.2
     !!1 CALL EvalBorderFaceXY_WE(ib,ix,vix,iy,iz,ibn,vjx,fac)
     CALL EvalBorderFaceYZ_WE(ib,ix,vix,iy,iz,ibn,vjx,fac)
     !!1 CALL EvalBorderFaceZX_WE(ib,ix,vix,iy,iz,ibn,vjx,fac)
     !............................................
...
SUBROUTINE EvalBordCell_NS(ib,ix,iy,viy,iz,ibn,vjy,fac)
  ...
  TYPE(CellP8_T) :: CellRa
     ...
     !Zuweisung Celle-Rand für Übergabe
     !Vertex-Folge entsprechend Point-Folge für VolP8-Berechnung
     !1) Orientation north-south-direction, these assignment is as well right
     !IF(fac==1) THEN !in north direction as well right
     ! CellRa%V0=Floor(ib)%Vertices(ix  ,viy    ,iz-1) !V-P0
     ! CellRa%V1=Floor(ib)%Vertices(ix  ,viy+fac,iz-1) !V-P1
     ! CellRa%V2=Floor(ib)%Vertices(ix-1,viy    ,iz-1) !V-P2
     ! CellRa%V3=Floor(ib)%Vertices(ix-1,viy+fac,iz-1) !V-P3
     ! CellRa%V4=Floor(ib)%Vertices(ix  ,viy    ,iz)  !V-P4
     ! CellRa%V5=Floor(ib)%Vertices(ix  ,viy+fac,iz)  !V-P5 
     ! CellRa%V6=Floor(ib)%Vertices(ix-1,viy    ,iz)  !V-P6
     ! CellRa%V7=Floor(ib)%Vertices(ix-1,viy+fac,iz)  !V-P7
     !ELSE IF(fac==-1) THEN ! in south direction appropriate
     ! CellRa%V0=Floor(ib)%Vertices(ix  ,viy+fac,iz-1) !V-P1
     ! CellRa%V1=Floor(ib)%Vertices(ix  ,viy    ,iz-1) !V-P0
     ! CellRa%V2=Floor(ib)%Vertices(ix-1,viy+fac,iz-1) !V-P3
     ! CellRa%V3=Floor(ib)%Vertices(ix-1,viy    ,iz-1) !V-P2
     ! CellRa%V4=Floor(ib)%Vertices(ix  ,viy+fac,iz)  !V-P5
     ! CellRa%V5=Floor(ib)%Vertices(ix  ,viy    ,iz)  !V-P4 
     ! CellRa%V6=Floor(ib)%Vertices(ix-1,viy+fac,iz)  !V-P7
     ! CellRa%V7=Floor(ib)%Vertices(ix-1,viy    ,iz)  !V-P6
     !END IF
     !2) Orientation coordinate system
     if(fac==-1) THEN !south-direction
       CellRa%V0=Floor(ib)%Vertices(ix-1,viy+fac,iz-1)  !V-P3
       CellRa%V1=Floor(ib)%Vertices(ix  ,viy+fac,iz-1)  !V-P1
       CellRa%V2=Floor(ib)%Vertices(ix-1,viy    ,iz-1)  !V-P2
       CellRa%V3=Floor(ib)%Vertices(ix  ,viy    ,iz-1)  !V-P0
       CellRa%V4=Floor(ib)%Vertices(ix-1,viy+fac,iz)   !V-P7
       CellRa%V5=Floor(ib)%Vertices(ix  ,viy+fac,iz)   !V-P5
       CellRa%V6=Floor(ib)%Vertices(ix-1,viy    ,iz)   !V-P6
       CellRa%V7=Floor(ib)%Vertices(ix  ,viy    ,iz)   !V-P4
     else if(fac==+1) THEN !north direction
       CellRa%V0=Floor(ib)%Vertices(ix-1,viy    ,iz-1) !V-P2
       CellRa%V1=Floor(ib)%Vertices(ix  ,viy    ,iz-1) !V-P0
       CellRa%V2=Floor(ib)%Vertices(ix-1,viy+fac,iz-1) !V-P3
       CellRa%V3=Floor(ib)%Vertices(ix  ,viy+fac,iz-1) !V-P1
       CellRa%V4=Floor(ib)%Vertices(ix-1,viy    ,iz)    !V-P6
       CellRa%V5=Floor(ib)%Vertices(ix  ,viy    ,iz)    !V-P4
       CellRa%V6=Floor(ib)%Vertices(ix-1,viy+fac,iz)    !V-P7
       CellRa%V7=Floor(ib)%Vertices(ix  ,viy+fac,iz)    !V-P5
     end if
     !.........................................
     !If(ib==3.AND.ix==3.AND.iy==100.AND.iz==8.AND.ibn==2) THEN
     !    Write(*,*)"ib==3.AND.ix==3.AND.iy==100", "iz=",iz,"  ibn=",ibn
     !    !Bsp-Check-south: Valley3D_4B_Osw.grid   Version 8.9.2.3 
     !    Write(*,*)"CellRa%V0%Point=",CellRa%V0%Point
     !    Write(*,*)"CellRa%V1%Point=",CellRa%V1%Point
     !    Write(*,*)"CellRa%V2%Point=",CellRa%V2%Point
     !    Write(*,*)"CellRa%V3%Point=",CellRa%V3%Point
     !    Write(*,*)"CellRa%V4%Point=",CellRa%V4%Point
     !    Write(*,*)"CellRa%V5%Point=",CellRa%V5%Point
     !    Write(*,*)"CellRa%V6%Point=",CellRa%V6%Point
     !    Write(*,*)"CellRa%V7%Point=",CellRa%V7%Point
     !end if
     !If(ib==2.AND.ix==3.AND.iy==101.AND.iz==8.AND.ibn==3) THEN
     !    Write(*,*)"ib==2.AND.ix==3.AND.iy==101", "iz=",iz,"  ibn=",ibn
     !    !Bsp-Check-north:  Valley3D_4B_Osw.grid   Version 8.9.2.3 
     !    Write(*,*)"CellRa%V0%Point=",CellRa%V0%Point
     !    Write(*,*)"CellRa%V1%Point=",CellRa%V1%Point
     !    Write(*,*)"CellRa%V2%Point=",CellRa%V2%Point
     !    Write(*,*)"CellRa%V3%Point=",CellRa%V3%Point
     ...
     END IF
     !............................................
     !!Uebernehme Koordinate der akt RandCelle p-> Berechne FacesRand
     !!1 !getestet 26.11.2009,VGrid_GMV_8.4.1.2
     !!1 CALL EvalBorderFaceXY_NS(ib,ix,iy,viy,iz,ibn,vjy,fac)
     !!1 CALL EvalBorderFaceYZ_NS(ib,ix,iy,viy,iz,ibn,vjy,fac)
...
SUBROUTINE EvalBordCell_TB(ib,ix,iy,iz,viz,ibn,vjz,fac)
  ...
  TYPE(CellP8_T) :: CellRa
    ...
    !if(fac==+1) THEN  !top-direction
    ! CellRa%V0=Floor(ib)%Vertices(ix  ,iy  ,viz)     !V-P0
    ! CellRa%V1=Floor(ib)%Vertices(ix  ,iy  ,viz+fac) !V-P1
    ! CellRa%V2=Floor(ib)%Vertices(ix-1,iy  ,viz)     !V-P2
    ! CellRa%V3=Floor(ib)%Vertices(ix-1,iy  ,viz+fac) !V-P3
    ! CellRa%V4=Floor(ib)%Vertices(ix  ,iy-1,viz)     !V-P4
    ! CellRa%V5=Floor(ib)%Vertices(ix  ,iy-1,viz+fac) !V-P5
    ! CellRa%V6=Floor(ib)%Vertices(ix-1,iy-1,viz)     !V-P6
    ! CellRa%V7=Floor(ib)%Vertices(ix-1,iy-1,viz+fac) !V-P7
    !else if (fac==-1) THEN !botton-direction
    ! CellRa%V0=Floor(ib)%Vertices(ix  ,iy  ,viz+fac)  !V-P1
    ! CellRa%V1=Floor(ib)%Vertices(ix  ,iy  ,viz)      !V-P0
    ! CellRa%V2=Floor(ib)%Vertices(ix-1,iy  ,viz+fac)  !V-P3
    ! CellRa%V3=Floor(ib)%Vertices(ix-1,iy  ,viz)      !V-P2
    ! CellRa%V4=Floor(ib)%Vertices(ix  ,iy-1,viz+fac)  !V-P5
    ! CellRa%V5=Floor(ib)%Vertices(ix  ,iy-1,viz)      !V-P4
    ! CellRa%V6=Floor(ib)%Vertices(ix-1,iy-1,viz+fac)  !V-P7
    ! CellRa%V7=Floor(ib)%Vertices(ix-1,iy-1,viz)      !V-P6
    !end if
    !2) Orientation coordinate system
    if(fac==-1) THEN !botton-direction
     CellRa%V0=Floor(ib)%Vertices(ix-1,iy-1,viz+fac) !V-P7
     CellRa%V1=Floor(ib)%Vertices(ix  ,iy-1,viz+fac) !V-P5
     CellRa%V2=Floor(ib)%Vertices(ix-1,iy  ,viz+fac) !V-P3
     CellRa%V3=Floor(ib)%Vertices(ix  ,iy  ,viz+fac) !V-P1
     CellRa%V4=Floor(ib)%Vertices(ix-1,iy-1,viz)     !V-P6
     CellRa%V5=Floor(ib)%Vertices(ix  ,iy-1,viz)     !V-P4
     CellRa%V6=Floor(ib)%Vertices(ix-1,iy  ,viz)     !V-P2
     CellRa%V7=Floor(ib)%Vertices(ix  ,iy  ,viz)     !V-P0
    else if(fac==+1) THEN !top-direction
     CellRa%V0=Floor(ib)%Vertices(ix-1,iy-1,viz)     !V-P6
     CellRa%V1=Floor(ib)%Vertices(ix  ,iy-1,viz)     !V-P4
     CellRa%V2=Floor(ib)%Vertices(ix-1,iy  ,viz)     !V-P2
     CellRa%V3=Floor(ib)%Vertices(ix  ,iy  ,viz)     !V-P0
     CellRa%V4=Floor(ib)%Vertices(ix-1,iy-1,viz+fac) !V-P7
     CellRa%V5=Floor(ib)%Vertices(ix  ,iy-1,viz+fac) !V-P5
     CellRa%V6=Floor(ib)%Vertices(ix-1,iy  ,viz+fac) !V-P3
     CellRa%V7=Floor(ib)%Vertices(ix  ,iy  ,viz+fac) !V-P1
    end if
    !.........................................
    CALL AnalyzeCellRand(CellRa)
    !IF(CellRa%in_out>-8.AND.CellRa%in_out<6) THEN  ?NrMP nein NrW ja?
        ...
        ELSE  !!'iw'.OR.'ie' 
          ...
          !!      -------------
            ...
            !      END IF
                  ...
                  !    END IF  
                  !  END IF
                  !  !!!Variante mit Nachbarstruktur
                  !  !!!Floor(ib)%Ngbr_blk_rc[1]%Cell(ix,iy,iz)%Cell%Vol=
                  !  !!!Floor(ib)%Nachbars[1]%Cell(ix,iy,iz)%Cell%Vol=
                  !END IF
             !  END DO
             ! END DO
            ELSE  !!(RefineY<RefineNachbarY)
        ...
        DO j=iy0+1,iy1
         ...
         DO k=iz0+1,iz1
          IF (.NOT.ASSOCIATED(Cell(i,j,k)%Cell)) THEN
            ...
            !w   !Write(*,*) " EXIT aus Zählung NrRN_Cells w/e AnalyzeAllRandCells"
            !w   !Write(*,*) " Randzählung : i,j,k=",i,j,k
            !w   Write(OutUnitProt,*) " Info : EXIT aus Zählung NrRN_Cells w/e AnalyzeAllRandCells"
      ...
      CASE ("n","s")      !? pn,ps !? läüft  unter else wie 'in,is'
        ...
        DO i=ix0+1,ix1
          ...
          IF (.NOT.ASSOCIATED(Cell(i,j,k)%Cell)) THEN
           ...
           !w  ! Write(*,*) " EXIT aus Zählung NrRN_Cells n/s AnalyzeAllRandCells"
           !w  ! Write(*,*) " iandzählung : i,j,k=",i,j,k
           !w   Write(OutUnitProt,*) " Info : EXIT aus Zählung NrRN_Cells n/s AnalyzeAllRandCells"
           !w   Write(OutUnitProt,*) "        Randzählung : i,j,k=",i,j,k,"  Block : ",ib
...
!27.10.11                NrRN_Cells=NrRN_Cells+1
              IF (ASSOCIATED(Cell(ix,iy,iz0+1)%Cell)) THEN
                 !Floor(ib)%Cell(ix,iy,iz1+1)%Cell=>Floor(ib)%Cell(ix,iy,iz1)%Cell
                 !IF(Cell(ix,iy,iz1)%Cell%vc>0) THEN
                 IF(Cell(ix,iy,iz0+1)%Cell%in_out<6.AND.Cell(ix,iy,iz0+1)%Cell%Vol>0.0d0.OR. &
...
END SUBROUTINE AnalyzeAllRandCells
...
SUBROUTINE SortVertCutCell(Cell,ix,iy,iz)
  ...
  IF(Cell%vc>0) THEN
   ...
   !         ListCut(2,j)=iTemp
       ...
       ELSE
          IF(vc_out1<nCut) THEN
             DEALLOCATE(Cell%VertCut)
             ALLOCATE(Cell%VertCut(nCut))
          END IF
       END IF
       
       IF (nCut.NE.nCutNeu) THEN
          Write(*,*) "nCut=",nCut,"    nCutNeu=",nCutNeu,"  ix=",ix,"  iy=",iy,"  iz=",iz
       END IF
       Cell%VertCut(1)= ListCut(1,1)
       Cell%VertCut(2)= ListCut(2,1)
       ListCut(1,1)=0
  ...
  ELSE  !vc==0
    IF (Cell%Face1%in_out==0.AND.(Cell%Face2%in_out>=2.AND.Cell%Face2%in_out<=4)) THEN 
       !Grenzflaeche Face1,und(Face2%in_out==4 od.
       !Grenzflaeche Face1,und Edge-Grenze , Grenzflaeche Face1,und Point-Grenze
       IF (.NOT.ASSOCIATED(Cell%VertCut)) THEN
          ALLOCATE(Cell%VertCut(Cell%Face1%NumberVert))
       END IF
       DO iv=1,Cell%Face1%NumberVert
         Cell%VertCut(iv)=Cell%Face1%VertexList(iv)
       END DO
    ELSE  !wenn Face 3-6  in_out==4
      IF (Cell%Face3%in_out==0) THEN
        IF (.NOT.ASSOCIATED(Cell%VertCut)) THEN
           ALLOCATE(Cell%VertCut(Cell%Face3%NumberVert))
        END IF
        DO iv=1,Cell%Face3%NumberVert
          Cell%VertCut(iv)=Cell%Face3%VertexList(iv)
        END DO
      ELSE IF (Cell%Face4%in_out==0) THEN
        IF (.NOT.ASSOCIATED(Cell%VertCut)) THEN
           ALLOCATE(Cell%VertCut(Cell%Face4%NumberVert))
        END IF
        DO iv=1,Cell%Face4%NumberVert
          Cell%VertCut(iv)=Cell%Face4%VertexList(iv)
        END DO
      ELSE IF (Cell%Face5%in_out==0) THEN
        IF (.NOT.ASSOCIATED(Cell%VertCut)) THEN
           ALLOCATE(Cell%VertCut(Cell%Face5%NumberVert))
        END IF
        DO iv=1,Cell%Face5%NumberVert
          Cell%VertCut(iv)=Cell%Face5%VertexList(iv)
        END DO
      ELSE IF (Cell%Face6%in_out==0) THEN
        IF (.NOT.ASSOCIATED(Cell%VertCut)) THEN
           ALLOCATE(Cell%VertCut(Cell%Face6%NumberVert))
        END IF
        DO iv=1,Cell%Face6%NumberVert
          Cell%VertCut(iv)=Cell%Face6%VertexList(iv)
        END DO
      END IF
    END IF
  END IF  ! If(vc>0) else 
  !Zaehlung Cut-Cellen (alle geschnittenen und GrenzCellen am Berg)
  IF (Cell%in_out>-8.AND.Cell%in_out<8 )THEN
    nr_cutinside=nr_cutinside+1
  END IF
...
!-------------------------------------------------------------------------
! Display Read and Grid-Dists-Control
!-------------------------------------------------------------------------
...
SUBROUTINE  Prot_Result_ReadGrid
  ...
  WRITE(OutUnitProt,*) leerzei2,'           >>>>   Result  Read-Grid     <<<<'
  WRITE(OutUnitProt,*) trenn2 
  WRITE(OutUnitProt,*)
  ...
  WRITE(OutUnitProt,*)
  WRITE(OutUnitProt,*) leerzei2,'    Shrinks points of volume-analysis  : ',Shrink
  WRITE(OutUnitProt,*)
  ...
  WRITE(OutUnitProt,*) &
    ...
    " |  Shrink         =",Shrink, &
        & " ! shrinks points of volume-analysis (0 für Einheitswürfel)\n", &
    " |  IncrVol        =",IncrVol, &
  ...
  Write(OutUnitProt,*) &
    ...
    " ------------------------------------------------------------------"
  WRITE(OutUnitProt,*)
  WRITE(OutUnitProt,*) leerzei2,'           >>>>  End  Result  Read-Grid     <<<<'
  WRITE(OutUnitProt,*) trenn2 
  WRITE(OutUnitProt,*)

END SUBROUTINE  Prot_Result_ReadGrid
  
SUBROUTINE WriteFaceProt(Face)
  TYPE(Face_T), POINTER :: Face
...
SUBROUTINE WriteCellProt(Cell,i,j,k)
  ...
  IF (ASSOCIATED(Cell)) THEN
    ...
    CALL WriteFaceProt(Cell%Face1)
    WRITE(OutUnitProt,*) '............'
    WRITE(OutUnitProt,*) ' --> FaceXY','  (F2)'
    CALL WriteFaceProt(Cell%Face2)
    WRITE(OutUnitProt,*) '............'
    WRITE(OutUnitProt,*) ' --> FaceXZ','  (F3)'
    CALL WriteFaceProt(Cell%Face3)
    WRITE(OutUnitProt,*) '............'
    WRITE(OutUnitProt,*) ' --> FaceXZ','  (F4)'
    CALL WriteFaceProt(Cell%Face4)
    WRITE(OutUnitProt,*) '............'
    WRITE(OutUnitProt,*) ' --> FaceYZ','  (F5)'
    CALL WriteFaceProt(Cell%Face5)
    WRITE(OutUnitProt,*) '............'
    WRITE(OutUnitProt,*) ' --> FaceYZ','  (F6)'
    CALL WriteFaceProt(Cell%Face6)
    WRITE(OutUnitProt,*) 'Cell','(',i,',',j,',',k,')'
