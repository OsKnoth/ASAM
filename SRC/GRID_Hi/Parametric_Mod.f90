...
! !Input_Dists_Ctrl-Part
!  REAL(8) :: dist_fscv     ! for fine scaling dist to in_out-def, CheckVertex 
!  REAL(8) :: distx_coeff      ! Distance Point(x) coefficient
!  REAL(8) :: disty_coeff      ! Distance Point(y) coefficient
!  REAL(8) :: distz_coeff      ! Distance Point(z) coefficient
!  REAL(8) :: dxViewLoc      ! Distance Point(x) coefficient of model border
!  REAL(8) :: dyViewLoc      ! Distance Point(y) coefficient of model border
!  REAL(8) :: dzViewLoc      ! Distance Point(z) coefficient of model border
!  REAL(8) :: Shrink         ! shrinks points of volume-analysis (0 für Einheitswürfel)
!  INTEGER :: IncrVol            ! counter for cuts of volume-analysis (1 für Einheitswürfel 
...
!  INTEGER :: IncrVol=10            ! counter for cuts of volume-analysis (1 für Einheitswürfel 
  NAMELIST /GridDistsCtrl/ dist_fscv, distx_coeff, disty_coeff, distz_coeff, &
                           dxViewLoc, dyViewLoc, dzViewLoc, Shrink, IncrVol, &
                           dist_scMaxCell

  NAMELIST /OutGMVControl/ out_wahlgrid, out_type, conv_ingrid, invalue_to_out, RadOutput, ScaleRad, ScaleSoil
  ...
  SELECT CASE (OutGrid)
    ...
    CASE ('Cart')
      ...
      ELSE IF (invalue_to_out=='y'.AND. &
               (TRIM(indata_type)==geographical &
                .OR. &
                TRIM(indata_type)==wgs84)) THEN
        ! IF TRIM(indata_type)==geographical) THEN
           xParametricOut=xParametricRadToGeo(Vertex)
        ! ELSE IF
        ! ELSE
        ! END IF
      ELSE
    ...
    CASE ('Cart')
      ...
      ELSE IF (invalue_to_out=='y'.AND. &
               (TRIM(indata_type)==geographical &
                ...
                TRIM(indata_type)==wgs84)) THEN
        ! IF TRIM(indata_type)==geographical) THEN
           yParametricOut=yParametricRadToGeo(Vertex)
        ! ELSE IF
        ! ELSE
        ! END IF
      ELSE
